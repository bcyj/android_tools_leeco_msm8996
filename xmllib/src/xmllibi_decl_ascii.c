...
DESCRIPTION
 This file implements the XML decl library in ascii encoding.
...
/*===========================================================================
  ...
  $Header: //depot/asic/sandbox/projects/cne/common/xmllib/src/xmllibi_decl_ascii.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/29/06   ssingh  Moved typedefs and macros from the xmllibi_decl_ascii.h 
                   file to here 
...
/*===========================================================================
                         local definitions
===========================================================================*/
#define XMLLIB_SINGLE_QUOTE_ASCII 39
#define XMLLIB_DOUBLE_QUOTE_ASCII 34
...
/*---------------------------------------------------------------------------
  typedef of xmllib_tok_ascii const-->string mapping table
---------------------------------------------------------------------------*/
typedef struct
{ 
  char                     * str;
  xmllib_quote_req_e_type    quote_req;
} xmllib_tok_ascii_const_tbl_type ;
...
static const xmllib_tok_ascii_const_tbl_type const_tbl[XMLLIB_CONST_MAX] =
{
...
)
{
  ...
  switch(const_tbl[expval].quote_req)
  {
    case XMLLIB_TEXT_QUOTE_REQ:
      if( XMLLIB_DOUBLE_QUOTE_ASCII != string[0] && 
          XMLLIB_SINGLE_QUOTE_ASCII != string[0] )
      {
        ret = XMLLIB_ERROR;
  ...
  if( TRUE == quotes_exist )
  {
    string++;
    source_len -= 2;
    if( XMLLIB_DOUBLE_QUOTE_ASCII != source->string[source->len-1] && 
        XMLLIB_SINGLE_QUOTE_ASCII != source->string[source->len-1] )
    {
      ret = XMLLIB_ERROR;
      *error_code = XMLLIB_ERROR_UNEXPECTED_TEXT ;
    }
  ...
  return ret;
} /* xmllib_decl_ascii_check_value_const() */
