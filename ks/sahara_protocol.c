 ...
 * ==========================================================================================
 *   $Header: //source/qcom/qct/core/storage/tools/kickstart/common/kickstart/kickstart/sahara_protocol.c#2 $
...
bool sahara_rx_data(size_t bytes_to_read, bool use_timeout) {
    bool retval = false;
    ...
    if (0 == bytes_to_read) {
        if (false == rx_data(sahara_data.rx_buffer, sizeof(sahara_packet_header), &temp_bytes_read, use_timeout)) {
            return false;
        }
        else {
    ...
    while (bytes_read < bytes_to_read) {
        if (false == rx_data(sahara_data.rx_buffer + bytes_read, bytes_to_read - bytes_read, &temp_bytes_read, use_timeout))
            return false;
        else
            bytes_read += temp_bytes_read;
    }
...
bool sahara_start() {
    ...
    while (1)
    {
        ...
        case SAHARA_WAIT_HELLO:
          ...
          if (false == sahara_rx_data(0, false))
            return false;
          ...
          if (SAHARA_HELLO_ID != sahara_hello->header.command) {
              ...
              sahara_hello_resp->version = SAHARA_VERSION;
              sahara_hello_resp->version_supported = SAHARA_VERSION_SUPPORTED;
        ...
        case SAHARA_WAIT_COMMAND:
            if (false == sahara_rx_data(0, true))
               return false;
            ...
            if (true == is_end_of_image_rx (((sahara_packet_header *)sahara_data.rx_buffer)->command)) {
                ...
                if (true == is_ack_successful (sahara_end_image_tx->status)) {
                    ...
                    return false; // No longer sending reset, need to send error up to MDM Helper level
                    /*
                    dbg(LOG_EVENT, "SENDING --> SAHARA_RESET");
                    if (false == send_reset_command ()) {
                        dbg(LOG_ERROR, "Sending RESET packet failed");
                ...
                if (false == started_raw_image_transfer) {
                    ...
                    if (NULL == id_mapped_file) {
                        ...
                        sahara_data.fd = open_file(id_mapped_file, true, 0);
        ...
        case SAHARA_WAIT_MEMORY_TABLE:
            if (memory_table_length > 0) {
                if (false == sahara_rx_data(memory_table_length, true)) {
                   return false;
                }
                dbg(LOG_INFO, "Memory Debug table received");

				if (true == sahara_data.ram_dump_64bit) {
					memcpy (sahara_data.misc_buffer, sahara_data.rx_buffer, memory_table_length);
					num_debug_entries = memory_table_length/sizeof(dload_debug_type_64bit);
				}
				else {
					num_debug_entries = memory_table_length/sizeof(dload_debug_type);
        ...
        case SAHARA_WAIT_MEMORY_REGION:
            ...
            if (sahara_data.state == SAHARA_WAIT_MEMORY_REGION)
            {
                if (false == sahara_rx_data(memory_table_length, true)) {
                    ...
                    dbg(LOG_WARN, "Retrying memory read request");
                }
                else {
                    num_dump_failures = 0;
                    ...
                    retval = write(sahara_data.fd, sahara_data.rx_buffer, memory_table_length);
                    if (retval < 0) {
                    ...
                    if (sahara_memory_table[num_debug_entries].length == 0)
                    {
                        ...
                        {
							sahara_data.fd = open_file(sahara_memory_table[num_debug_entries].filename, false, sahara_memory_table[num_debug_entries].length);
#ifdef WINDOWSPC
							_lseek(sahara_data.fd, sahara_memory_table[num_debug_entries].length - 1, SEEK_SET);
            ...
            if (num_debug_entries >= 0)
            {
                ...
                sahara_data.timed_data_size += memory_table_length;
            }
            else {
        ...
        case SAHARA_WAIT_DONE_RESP:
          ...
          if (SAHARA_MODE_IMAGE_TX_PENDING == sahara_done_resp->image_tx_status) {
           ...
           dbg(LOG_ERROR, "Received unrecognised status %d at SAHARA_WAIT_DONE_RESP state",  sahara_done_resp->image_tx_status);
           return false;
          }
        ...
        case SAHARA_WAIT_RESET_RESP:
            if (true == sahara_data.allow_sahara_reset) {
                if (true == sahara_rx_data(0, true)) {
                    if (SAHARA_RESET_RESP_ID != sahara_reset_resp->header.command) {
                        dbg(LOG_WARN,"Waiting for reset response code %i, received %i instead.", SAHARA_RESET_RESP_ID, sahara_reset_resp->header.command);
                        continue;
                    }
                }
                else {
            ...
            {
                dbg(LOG_STATUS, "Skipping SAHARA_WAIT_RESET_RESP");
                return true;
            }
        ...
        default:
            dbg(LOG_ERROR, "Unrecognised state %d",  sahara_data.state);
            return false;
        } /* end switch */
    } /* end while (1) */
}
...
static bool send_reset_command ()
{
    ...
    // Check if reset should be sent.
    if (false == sahara_data.allow_sahara_reset) {
        dbg(LOG_STATUS, "Sending sahara reset pkt disabled by '-n or --noreset'");
        return true;
    }

    sahara_reset = (sahara_packet_reset *)sahara_data.tx_buffer;
...
static bool start_image_transfer ()
{
    ...
    dbg(LOG_INFO, "start_image_transfer: Offset %d", sahara_read_data->data_offset);
    /*check if the input request is valid*/
    if ((0 == sahara_read_data->data_length) ||
        (is_reg_file == 1  &&  (sahara_read_data->data_offset + sahara_read_data->data_length) > get_file_size(sahara_data.fd))) {
          dbg(LOG_ERROR, "Invalid length %d bytes request to be transmitted", sahara_read_data->data_length);
          return false;
    }

    /*read data to buffer based on the offset*/
    lseek(sahara_data.fd, sahara_read_data->data_offset, SEEK_SET);
    while (bytes_read < sahara_read_data->data_length) {
		bytes_to_read_next = MIN(sahara_read_data->data_length - bytes_read, SAHARA_RAW_BUFFER_SIZE);
		...
		if ((uint32_t) retval != bytes_to_read_next) {
			dbg(LOG_ERROR, "Read %d bytes, but was asked for %d bytes", retval, sahara_read_data->data_length);
			return false;
		}
    ...
    sahara_data.timed_data_size += sahara_read_data->data_length;
    return true;
}
...
static bool send_memory_read_packet (uint64_t memory_table_address, uint64_t memory_table_length)
{
	...
	if (true == sahara_data.ram_dump_64bit) {
		...
		sahara_memory_read->memory_addr = memory_table_address;
		sahara_memory_read->memory_length = memory_table_length;
...
static int send_cmd_exec_command (unsigned int command)
{
    ...
    if (sahara_cmd_exec_resp->header.command == SAHARA_CMD_EXEC_RESP_ID) {
        ...
        if (sahara_cmd_exec_resp->resp_length > 0) {
            ...
            // Retreive data
            if (false == sahara_rx_data(response_length, true)) {
                return false;
            }
			...
			if (response_length > 0) {
				fd = open_file("commandop.bin", false, response_length);
				if (-1 == fd) {
