 ...
 *
 *
 *  crc.c : 16-bit LSB CRC computation/verification
 * ==========================================================================================
 *   $Header: //source/qcom/qct/core/storage/tools/kickstart/crc.c#5 $
 ...
 *
 *  PARAMETERS:
 *  buffer       [ I ] - The data buffer
 *  length     [ I ] - The length of the above buffer
 *
 *  RETURN VALUE:
 *  None
 *  ===========================================================================*/
...
{
    /* Calculate CRC */
    unsigned short crc = calculate_crc (buffer, length * 8);

    /* Add CRC to data */
    buffer[length] = (unsigned char)(crc);
    buffer[length + 1] = (unsigned char)(crc >> 8);
}

/*===========================================================================
 *  METHOD:
 *  check_crc (Free Method)
 *
 *  DESCRIPTION:
 ...
 *  RETURN VALUE:
 *  bool:
 *     SUCCESS  - CRC checks out OK
 *     EFAILED - CRC doesn't cut the mustard
 *  ===========================================================================*/
int check_crc (char *buffer, unsigned long length)
{
    ...
    unsigned short check = CRC_16_L_OK;
 ...
 *
 *  RETURN VALUE:
 *  unsigned short : The 16-bit calculated CRC
 *  ===========================================================================*/
unsigned short  calculate_crc (char *pBuf, unsigned long bitLen)
{
    ...
    for (crc = CRC_16_L_SEED; bitLen >= 8; bitLen -= 8, pBuf++)
        crc = crc_table[(crc ^ *pBuf) & 0x00ff] ^ (crc >> 8);
    return ~crc;
}
