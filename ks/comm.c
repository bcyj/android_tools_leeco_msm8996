 ...
 * ==========================================================================================
 *   $Header: //source/qcom/qct/core/storage/tools/kickstart/common/kickstart/kickstart/comm.c#2 $
...
bool rx_data(byte *buffer, size_t bytes_to_read, size_t *bytes_read, bool use_timeout)
{
#ifdef WINDOWSPC
	DWORD retval;
    ...
    tv.tv_sec  = com_port.rx_timeout_sec;
    tv.tv_usec = com_port.rx_timeout_usec;

    retval = select (com_port.port_fd + 1, &rfds, NULL, NULL, ((true == use_timeout) ? (&tv) : (NULL)));
    if (0 == retval) {
