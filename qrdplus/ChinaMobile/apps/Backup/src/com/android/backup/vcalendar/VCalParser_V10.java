...
import com.android.backup.pim.VParser;

public class VCalParser_V10 extends VParser {
    private static String TAG = "VCalParser_V10";
    /*
    ...
    private static final HashSet<String> mEvtPropNameGroup1 = new HashSet<String>(
            Arrays.asList("ATTACH", "ATTENDEE", "DCREATED", "COMPLETED",
                    ...
                    "DTSTAMP", // the new property
                    // above belong to simprop
                    "CLASS", "STATUS", "DURATION", "X-ALLDAY"));
    ...
    private static final HashSet<String> mValueCAT = new HashSet<String>(Arrays
            .asList("APPOINTMENT", "ANNIVERSARY", "BUSINESS", "EDUCATION", "HOLIDAY",
    ...
    private static final HashSet<String> mValueSTAT = new HashSet<String>(Arrays
            .asList("ACCEPTED", "NEEDS-ACTION", "NEEDS ACTION", "SENT", "TENTATIVE",
    ...
    private int parseParam(int offset) {
        ...
        // sunxiaoming add 20120903
        ret = parseParam21(offset);
        if (PARSE_ERROR != ret) {
            return ret;
        }
        // sunxiaoming add 20120903 End
        int start = offset;
     ...
     * seperated by ";" [ws] simprop [params] ":" value CRLF
     */
    private int parseEntprop0(int offset) {
        ...
        if (propName.equalsIgnoreCase("RRULE")) {// sunxiaoming
            isRrule = true;
            curRrule = true;
            // mBuilder.isRRule=true;
        }
        ret = propName.length();
        offset += ret;
        sum += ret;
        ...
        if (mBuilder != null) {
            ...
            v.add(exportEntpropValue(propName, mBuffer.substring(start,
                    offset)));
            if (curRrule) {// sunxiaoming modify
                mBuilder.propertyValues(v, curRrule);
                curRrule = false;
            } else {
                mBuilder.propertyValues(v, curRrule);
            }
    ...
    // sunxiaoming add 20120903
    private int parseParam21(int offset) {
        int ret = 0, sum = 0, start = offset;
        boolean flag = false;

        ret = parseString(offset, "TZID", true);
        if (PARSE_ERROR != ret) {
            offset += ret;
            sum += ret;
            flag = true;
        ...
        start = offset;
        /*
         * ret = parsePEncodingVal(offset); if (PARSE_ERROR == ret) { return
         * PARSE_ERROR; }
         */
        int mRet = 0;
        for (int i = 0;; i++) {// sunxiaoming add 20120903
            if (mBuffer.charAt(start + i) == '\r') {
                // mRet = i;
                break;
            }
            if (mBuffer.charAt(start + i) == ':') {
                mRet = i;
            }
        }

        offset += mRet;// sunxiaoming add 20120903
        sum += mRet;// sunxiaoming add 20120903
        if (mBuilder != null) {
            mBuilder.propertyParamValue(mBuffer.substring(start, offset));
        }
        //
    ...
    private int parseKnownType(int offset) {
        ...
        // sunxiaoming add for unknown type TZID
        // ret = parseString(offset, "TZID", true);//sunxiaoming delete 20120903
        //
        ret = removeWs(offset);
        offset += ret;
        sum += ret;
        ...
        for (;;) {
            //
            if (tempNum > mBuffer.length()) {
                break;
            }
            if (mBuffer.charAt(offset) == ':') {
                //
                break;
            }
            offset++;
    ...
    private int parseStatuVal(int offset) {
        ...
        ret = parseString(offset, "NEEDS-ACTION", true);// zhl modify to adapte
                                                        // v2.0
        if (PARSE_ERROR != ret) {
            return ret;
        }
        // zhl modified
        ret = parseString(offset, "NEEDS ACTION", true);// v1.0
        if (PARSE_ERROR != ret) {
            return ret;
        }
     ...
     * client. Though vcalendar1.0 spec does not defined the value of property
     ...
     * properties.
     *
     * @param str: the value string will be translated.
