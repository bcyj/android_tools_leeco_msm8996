...
import com.android.backup.vcard.exception.VCardAgentNotSupportedException;
import com.android.backup.vcard.exception.VCardException;
import com.android.backup.vcard.exception.VCardInvalidCommentLineException;
import com.android.backup.vcard.exception.VCardInvalidLineException;
...
public class VCardParser_V21 extends VCardParser {
    ...
    public VCardParser_V21(VCardSourceDetector detector, Handler callbackHandler) {
        this(detector != null ? detector.getEstimatedType() : VCardConfig.PARSE_TYPE_UNKNOWN);
    ...
    private boolean parseOneVCard(boolean firstReading) throws IOException, VCardException {
        if (mCallBackHandler != null) {
            mCallBackHandler.sendMessage(mCallBackHandler.obtainMessage(
                    EVENT_SET_PROGRESS_VALUE, mContactCount++, 0));
        }
     ...
     * here. addressparts = 0*6(strnosemi ";") strnosemi ; PO Box, Extended
     * Addr, Street, Locality, Region, Postal Code, Country Name orgparts =
     ...
     * are not sure whether we should add "\" CRLF to each value. For now, we
     * exclude them.
     */
     ...
     * all. item = ... / [groups "."] "AGENT" [params] ":" vcard CRLF vcard =
     * "BEGIN" [ws] ":" [ws] "VCARD" [ws] 1*CRLF items *CRLF "END" [ws] ":" [ws]
     * "VCARD"
     */
