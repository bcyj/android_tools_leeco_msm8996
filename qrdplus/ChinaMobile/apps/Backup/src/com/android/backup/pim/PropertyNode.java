...
public class PropertyNode {
    ...
    /**
     * Store value as byte[],after decode. Used when propValue is encoded by
     * something like BASE64, QUOTED-PRINTABLE, etc.
     */
    public byte[] propValue_bytes;

    /**
     * param store: key=paramType, value=paramValue Note that currently
     * PropertyNode class does not support multiple param-values defined in
     * vCard 3.0 (See also RFC 2426). multiple-values are stored as one String
     * value like "A,B", not ["A", "B"]... TODO: fix this.
     */
    ...
    public boolean equals(Object obj) {
        ...
        if (propValue_bytes != null && Arrays.equals(propValue_bytes, node.propValue_bytes)) {
            ...
            // decoded by BASE64 or QUOTED-PRINTABLE. When the size of
            // propValue_vector
            // is 1, the encoded value is stored in propValue, so we do not have
            // to
            // check it.
            return (propValue_vector.equals(node.propValue_vector) ||
                    propValue_vector.size() == 1 || node.propValue_vector.size() == 1);
        }
    }
    ...
    public String encode() {
        // PropertyNode#toString() is for reading, not for parsing in the
        // future.
