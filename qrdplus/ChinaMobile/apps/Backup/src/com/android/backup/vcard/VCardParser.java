...
public abstract class VCardParser {
    ...
    /**
     * <P>
     * Parses the given stream and send the VCard data into VCardBuilderBase
     * object.
     * </P
     * .
     * <P>
     * Note that vCard 2.1 specification allows "CHARSET" parameter, and some
     * career sets local encoding to it. For example, Japanese phone career uses
     * Shift_JIS, which is formally allowed in VCard 2.1, but not recommended in
     * VCard 3.0. In VCard 2.1, In some exreme case, some VCard may have
     * different charsets in one VCard (though we do not see any device which
     * emits such kind of malicious data)
     * </P>
     * <P>
     * In order to avoid "misunderstanding" charset as much as possible, this
     * method use "ISO-8859-1" for reading the stream. When charset is specified
     * in some property (with "CHARSET=..." parameter), the string is decoded to
     * raw bytes and encoded to the charset. This method assumes that
     * "ISO-8859-1" has 1 to 1 mapping in all 8bit characters, which is not
     * completely sure. In some cases, this "decoding-encoding" scheme may fail.
     * To avoid the case,
     * </P>
     * <P>
     * We recommend you to use {@link VCardSourceDetector} and detect which kind
     * of source the VCard comes from and explicitly specify a charset using the
     * result.
     * </P>
     *
     * @param is The source to parse.
     * @param interepreter A {@link VCardInterpreter} object which used to
     *            construct data.
