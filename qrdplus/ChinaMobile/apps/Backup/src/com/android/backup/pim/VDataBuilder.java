...
package com.android.backup.pim;
...
public class VDataBuilder implements VBuilder {
    ...
    static private boolean LOCAL_DEBUG = true;
    private static final boolean mUseUTC = true;

    /**
     * If there's no other information available, this class uses this charset
     * for encoding byte arrays.
     */
    ...
    // When error occurs after calling startRecord(), the entry which is
    // probably
    // the cause of the error remains to be in vNodeList, while endRecord() is
    // not called.
    //
    // I leave this code as is since I'm not familiar with vcalendar
    // specification.
    ...
    public void startRecord(String type) {
        ...
        vnode.parseStatus = 1;
        vnode.VName = type;
        // I feel this should be done in endRecord(), but it cannot be done
        // because of
        // the reason above.
        vNodeList.add(vnode);
    ...
    private String handleOneValue(String value, String targetCharset, String encoding) {
        if (encoding != null) {
            if (encoding.equals("BASE64") || encoding.equals("B")) {
                ...
                try {
                    if (!isRRule) {// sunxiaoming modify for rrule
    ...
    public void propertyValues(List<String> values, boolean RRule) {
        // sunxiaoming modify for rrule
        isRRule = RRule;
    ...
    public void propertyValues(List<String> values) {
        ...
        String timeZone = paramMap.getAsString("TZID"); // sunxiaoming add
                                                        // 20120904

        if (mCurrentPropNode.propName.equalsIgnoreCase("DTSTART")/*
                                                                  * sunxiaoming
                                                                  * add 20120904
                                                                  */
                || mCurrentPropNode.propName.equalsIgnoreCase("DTEND")) {
            if (timeZone != null && timeZone.length() != 0) {

                PropertyNode mCurrentPropNode1 = new PropertyNode();
                mCurrentPropNode1.propName = "TZID";
                mCurrentPropNode1.propValue = timeZone;
                mCurrentVNode.propList.add(mCurrentPropNode1);
            }
        ...
        for (String value : values) {
            mCurrentPropNode.propValue_vector.add(
                    handleOneValue(value, mTargetCharset, encoding));
        }
