...
abstract public class VParser {
    ...
    private static String TAG = "VParser";
    // sdk 1.6 zhl add ..not know the charset clearly

    // Assume that "iso-8859-1" is able to map "all" 8bit characters to some
    // unicode and
    ...
    protected boolean isRrule = false;
     ...
     * @param builder The v builder which used to construct data.
     ...
     * string.
     *
     * @param offset The offset in buffer of current string
     * @param tar The given string.
     * @param ignoreCase To determine case sensitive or not.
    ...
    protected int parseQuotedPrintable(int offset) {
        int ret = 0, sum = 0;
        if (isRrule) {// sunxiaoming add for work with the "RRULE"
            for (;;) {
                if (mBuffer.charAt(offset) == '\r') {
                    //
                    isRrule = false;
                    return sum;
                }
    ...
    protected int parsePEncodingVal(int offset) {
        ...
        // sunxiaoming add start
        /*
         * ret = parseString(offset, "TZID", true); if (ret != PARSE_ERROR) {
         * mEncoding = "TZID"; sum += ret; return sum; }
         */
        // sunxiaoming add end
    ...
    protected int parseCharsetVal(int offset) {
        ...
        // zhl add for support utf-8 encode
        ret = parseString(offset, "UTF-8", true);
        if (ret != PARSE_ERROR) {
            sum += ret;
            return sum;
        }
