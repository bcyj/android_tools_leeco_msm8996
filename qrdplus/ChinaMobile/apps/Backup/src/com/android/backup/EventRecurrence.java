...
public class EventRecurrence {
    ...
    /**
     * If set, allow lower-case recurrence rule strings. Minor performance
     * impact.
     */
    ...
    /**
     * If set, require that only one of {UNTIL,COUNT} is present. Breaks compat
     * w/ old parser.
     */
    private static final boolean ONLY_ONE_UNTIL_COUNT = false;

    /**
     * Thrown when a recurrence string provided can not be parsed according to
     * RFC2445.
     */
     ...
     * constants. btw, I think we should switch to those here too, to get rid of
    ...
    /**
     * Determines whether this rule specifies a simple monthly rule by weekday,
     * such as "FREQ=MONTHLY;BYDAY=3TU" (the 3rd Tuesday of every month).
     * <p>
     * Negative days, e.g. "FREQ=MONTHLY;BYDAY=-1TU" (the last Tuesday of every
     * month), will cause "false" to be returned.
     * <p>
     * Rules that fire every week, such as "FREQ=MONTHLY;BYDAY=TU" (every
     * Tuesday of every month) will cause "false" to be returned. (Note these
     ...
     * <p>
     * The native implementation over-allocated the arrays (and may have stuff
     * left over from a previous run), so we can't just check the arrays -- the
     * separately-maintained count field also matters. We assume that a null
     * array will have a count of zero, and that the array can hold as many
     * elements as the associated count indicates.
     * <p>
    ...
    @Override
    public int hashCode() {
        // We overrode equals, so we must override hashCode(). Nobody seems to
        // need this though.
        throw new UnsupportedOperationException();
    }

    /**
     * Resets parser-modified fields to their initial state. Does not alter
     * startDate.
     * <p>
     * The original parser always set all of the "count" fields, "wkst", and
     * "until", essentially allowing the same object to be used multiple times
     * by calling parse(). It's unclear whether this behavior was intentional.
     * For now, be paranoid and preserve the existing behavior by resetting the
     * fields.
     * <p>
     * We don't need to touch the integer arrays; they will either be ignored or
     * overwritten. The "startDate" field is not set by the parser, so we ignore
     * it here.
     */
     ...
     * Attempting to parse malformed input will result in an
    ...
    public void parse(String recur) {
        /*
         * From RFC 2445 section 4.3.10: recur = "FREQ"=freq *( ; either UNTIL
         * or COUNT may appear in a 'recur', ; but UNTIL and COUNT MUST NOT
         * occur in the same 'recur' ( ";" "UNTIL" "=" enddate ) / ( ";" "COUNT"
         ...
         * ";" x-name "=" text ) ) The rule parts are not ordered in any
         * particular sequence. Examples:
         * FREQ=MONTHLY;INTERVAL=2;COUNT=10;BYDAY=1SU,-1SU
         * FREQ=YEARLY;INTERVAL=4;BYMONTH=11;BYDAY=TU;BYMONTHDAY=2,3,4,5,6,7,8
         * Strategy: (1) Split the string at ';' boundaries to get an array of
         * rule "parts". (2) For each part, find substrings for left/right sides
         * of '=' (name/value). (3) Call a <name>-specific parsing function to
         * parse the <value> into an output field. By keeping track of which
         * names we've seen in a bit vector, we can verify the constraints
         * indicated above (FREQ appears first, none of them appear more than
         * once -- though x-[name] would require special treatment), and we have
         * either UNTIL or COUNT but not both. In general, RFC 2445 property
         * names (e.g. "FREQ") and enumerations ("TU") must be handled in a
         * case-insensitive fashion, but case may be significant for other
         * properties. We don't have any case-sensitive values in RRULE, except
         * possibly for the custom "X-" properties, but we ignore those anyway.
         * Thus, we can trivially convert the entire string to upper case and
         * then use simple comparisons. Differences from previous version: -
         * allows lower-case property and enumeration values [optional] -
         * enforces that FREQ appears first - enforces that only one of UNTIL
         * and COUNT may be specified - allows (but ignores) X-* parts -
         * improved validation on various values (e.g. UNTIL timestamps) - error
         * messages are more specific TODO: enforce additional constraints
         * listed in RFC 5545, notably the "N/A" entries in section 3.3.10. For
         * example, if FREQ=WEEKLY, we should reject a rule that includes a
         * BYMONTHDAY part.
         */
        ...
        for (String part : parts) {
            ...
            /*
             * In lieu of a "switch" statement that allows string arguments, we
             * use a map from strings to parsing functions.
             */
    ...
    abstract static class PartParser {
        ...
        public static int parseIntRange(String str, int minVal, int maxVal, boolean allowZero) {
            try {
                if (str.charAt(0) == '+') {
                    // Integer.parseInt does not allow a leading '+', so skip it
                    // manually.
                    str = str.substring(1);
                }
         ...
         * @param maxVal Maximum allowed value.
         * @param allowZero Is 0 allowed?
         * @return A new array with values, sized to hold the exact number of
         *         elements.
         */
