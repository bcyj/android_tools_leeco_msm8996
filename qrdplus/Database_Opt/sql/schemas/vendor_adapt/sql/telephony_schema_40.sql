...
create unique sequence iccsms_id_seq as bigint in mmssms_db;
CREATE TABLE iccsms (
  "_id" INTEGER PRIMARY KEY default NEXT_VALUE OF iccsms_id_seq with check option,
  service_center_address nvarchar(50),
  address nvarchar(128),
  message_class nvarchar(20) DEFAULT 'UNKNOWN',
  body nvarchar(2048),
  date BIGINT,
  status INTEGER DEFAULT -1,
  index_on_icc INTEGER,
  is_status_report INTEGER,
  transport_type nvarchar(20) DEFAULT 'sms',
  type INTEGER,
  locked INTEGER DEFAULT 0,
  error_code INTEGER DEFAULT 0,
  sub_id INTEGER DEFAULT 0,
  status_on_icc INTEGER
) in mmssms_db;
...
call compatibility.set_or_create_pragma('mmssms_db','user_version','57');
...
CREATE TABLE carriers(
	...
	ppp_number nvarchar(128),
	localized_name nvarchar(128),
	v_mccmnc VARCHAR(40)
) in telephony_db;
...
create unique sequence calloption_id_seq as BIGINT in cdmacalloption_db;
CREATE TABLE calloption(
	"_id" BIGINT PRIMARY KEY default NEXT_VALUE OF calloption_id_seq with check option,
	name NVARCHAR(128),
	"numeric" VARCHAR(40),
	mcc varchar(10),
	mnc varchar(10),
	...
	type INTEGER,
	category INTEGER,
	state INTEGER) in cdmacalloption_db;

call compatibility.set_or_create_pragma('cdmacalloption_db','user_version','1');
