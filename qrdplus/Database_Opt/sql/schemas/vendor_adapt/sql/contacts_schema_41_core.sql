...
CREATE TABLE phone_lookup (
	...
	,normalized NVARCHAR(128) NOT NULL
...
create index search_index_name_digit_idx on search_index(name_digit for word_search);
...
create index raw_contacts_phone_bucket_label_idx on raw_contacts(phonebook_bucket, phonebook_label, sort_key, account_id);
...
SELECT  
	...
	contacts.starred AS starred,
	contacts.times_contacted AS times_contacted,
        name_accounts.account_name,
        name_accounts.account_type,
	(
        CASE
            WHEN photo_file_id IS NULL
            THEN (
...
JOIN raw_contacts AS name_raw_contact ON(name_raw_contact_id=name_raw_contact."_id")
JOIN
    accounts as name_accounts
ON
    (
        name_raw_contact.account_id =name_accounts."_id"
    )
;
