...
#ifndef __SDCDENGINE_H__
#define __SDCDENGINE_H__
...
status_t onOpenDecryptSession(int uniqueId,
                              DecryptHandle* decryptHandle,
                              int fd, int offset, int length, const char* mime);
#endif
...
/**
 * Session Class for Oma Drm Conversion. An object of this class is created
...
class ConvertSession {
    public :
        ...
        virtual ~ConvertSession() {
            if(filePath != NULL) {
                ALOGE("~ConvertSession:deleting filePath");
                delete[] filePath;
            }
        }
};
...
/**
 * Session Class for Oma Drm decoder. An object of this class is created
...
int  openDrmSession(const String8& path);

String8 getMetadataAsString(int uniqueId,  int sessionId);
int getRightsStatus(int uniqueId, const int sessionId, int action);
String8 getConstraintsAsString(int uniqueId, const int sessionId, int action);
String8 convertConstraintsToString(DrmConstraints* drmConstraints);

int mPlaybackStatus = -1;

/* FUJITSU:2013-01-10_NADEOR 0023997 DRM BUGFIX_MOD-S */
char* replaceStr(char *old);
/* FUJITSU:2013-01-10_NADEOR 0023997 DRM BUGFIX_MOD-E */
};
