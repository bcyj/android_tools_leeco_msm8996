...
void GameRotationVector::processResp(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    if(true != error ) {
        ...
        case SNS_SAM_GAME_RV_GET_ATTRIBUTES_RESP_V01:
            ...
            property_get(HAL_PROP_MAX_GAMERV, max_rate_prop_value, FREQ_GAMERV_DEF_HZ_STR);
...
void GameRotationVector::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        HAL_LOG_VERBOSE("%s: GAME RV: [0]: %f [1]: %f [2]: %f acc: %d ts: %u", __FUNCTION__,
                         la_sample.data[0], la_sample.data[1], la_sample.data[2],
