    ...
    if (true != error ) {
        if ( Utility::isGyroAvailable() ) {
            ...
            case SNS_SAM_ROT_VEC_GET_ATTRIBUTES_RESP_V01:
                ...
                property_get(HAL_PROP_MAX_ROTVEC, max_rate_prop_value, FREQ_ORIENT_DEF_HZ_STR);
...
void Orientation::processInd(sensor1_msg_header_s *msg_hdr, void *msg_ptr)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        HAL_LOG_VERBOSE("%s: ORIENTATION: [0]: %f [1]: %f [2]: %f acc: %d ts: %u",
                    __FUNCTION__, la_sample.data[0], la_sample.data[1],
                    la_sample.data[2], curr_sample->accuracy, curr_sample->timestamp);
        pthread_mutex_lock(&data_cb->data_mutex);
