...
typedef struct {
  ...
  color_proc_mod_t color_proc_mod;
  ...
  active_crop_mod_t active_crop_mod;
  bpc_mod_t bpc_mod;
  stats_mod_t stats;
} vfe_module_t;
...
typedef struct VFE_OperationConfigCmdType {
  ...
  VFE_StartConfigCmdType       startCmd;
...
typedef struct {
  ...
  VFE_OperationConfigCmdType vfe_op_cfg;
  current_output_info_t vfe_out;
  isp_stats_t vfe_stats_struct;
  mctl_ops_t *ops;
}vfe_ctrl_info_t;

#define VFE_MAX_OBJS 1
#define VFE_MAX_CLIENT_NUM 1
...
vfe_status_t is_vfe_black_level_reconfig(vfe_ctrl_info_t *p_obj,
   void *parm);
vfe_status_t is_vfe_asf_reconfig(vfe_ctrl_info_t *p_obj, void *parm);
vfe_status_t vfe_frame_skip_config_pattern(frame_skip_mod_t *mod, vfe_frame_skip *ext);
