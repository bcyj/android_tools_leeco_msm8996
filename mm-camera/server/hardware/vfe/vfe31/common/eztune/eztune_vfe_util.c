  ...
  vfemodule_t item, int32_t enable)
{
  ...
  switch(item) {
    ...
    case VFE_MODULE_LINEARIZATION:
      ...
      vfe_linearization_enable(&vfe_module->linear_mod,
        params, enable, TRUE);
      break;
#endif
    case VFE_MODULE_COLORCORRECTION:
      diagnostics->control_colorcorr.enable = enable;
      vfe_color_correct_enable(mod_id, &(vfe_module->color_correct_mod),
        params, enable, TRUE);
      break;
    case VFE_MODULE_COLORCONVERSION:
      diagnostics->control_colorconv.enable = enable;
      vfe_color_conversion_enable(mod_id, &vfe_module->chroma_enhan_mod, params,
        enable, TRUE);
      break;
    case VFE_MODULE_GAMMA:
      diagnostics->control_gamma.enable = enable;
      vfe_gamma_enable(mod_id, &(vfe_module->gamma_mod), params, enable, TRUE);
      break;
    ...
    case VFE_MODULE_BCC:
      ...
      vfe_demosaic_bcc_enable(&(vfe_module->bcc_mod),
        &(vfe_ctrl_obj->vfe_params), enable, TRUE);
      break;
#endif
    case VFE_MODULE_ABF:
    ...
    case VFE_MODULE_CHROMASUPPRESSION:
      ...
      vfe_chroma_suppression_enable(mod_id, &(vfe_module->chroma_supp_mod),
        params, enable, TRUE);
      break;
    case VFE_MODULE_MCE:
      diagnostics->control_memcolorenhan.enable = enable;
      vfe_mce_enable(mod_id, &(vfe_module->mce_mod),
        params, enable, TRUE);
      break;
    case VFE_MODULE_SCE:
      diagnostics->control_skincolorenhan.enable = enable;
      vfe_sce_enable(mod_id, &(vfe_module->sce_mod),
        params, enable, TRUE);
      break;
    ...
    case VFE_MODULE_CLFILTER: {
      ...
      if(enable)
        ...
        vfe_clf_enable(&(vfe_module->clf_mod), params, clf_enable, TRUE);
      break;}
#endif
    default:
  ...
  vfemodule_t item, int32_t cntrlenable)
{
  ...
  switch(item) {
    ...
    case VFE_MODULE_LINEARIZATION:
      ...
      vfe_linearization_trigger_enable(&vfe_module->linear_mod,
        params, cntrlenable);
      break;
#endif
    case VFE_MODULE_COLORCORRECTION:
      diagnostics->control_colorcorr.cntrlenable = cntrlenable;
      vfe_color_correct_trigger_enable(mod_id, &vfe_module->color_correct_mod,
        params, cntrlenable);
      break;
    case VFE_MODULE_COLORCONVERSION:
      diagnostics->control_colorconv.cntrlenable = cntrlenable;
      vfe_color_conversion_trigger_enable(mod_id, &vfe_module->chroma_enhan_mod,
        params, cntrlenable);
      break;
    case VFE_MODULE_GAMMA:
      diagnostics->control_gamma.cntrlenable = cntrlenable;
      vfe_gamma_trigger_enable(mod_id, &vfe_module->gamma_mod, params,
        cntrlenable);
      break;
    case VFE_MODULE_BLACKLEVEL:
    ...
    case VFE_MODULE_LUMAADAPTATION:
      ...
      vfe_la_trigger_enable(mod_id, &vfe_module->la_mod,
        params, cntrlenable);
      break;
    case VFE_MODULE_CHROMASUPPRESSION:
      diagnostics->control_chromasupp.cntrlenable = cntrlenable;
      vfe_chroma_suppression_trigger_enable(mod_id,
        &vfe_module->chroma_supp_mod, params, cntrlenable);
      break;
    case VFE_MODULE_MCE:
      diagnostics->control_memcolorenhan.cntrlenable = cntrlenable;
      vfe_mce_trigger_enable(mod_id, &vfe_module->mce_mod, params, cntrlenable);
      break;
    case VFE_MODULE_SCE:
      diagnostics->control_skincolorenhan.cntrlenable = cntrlenable;
      vfe_sce_trigger_enable(mod_id, &vfe_module->sce_mod, params, cntrlenable);
      break;
    case VFE_MODULE_DEMUX:
      diagnostics->control_demux.cntrlenable = cntrlenable;
      vfe_demux_trigger_enable(mod_id, &vfe_module->demux_mod, params,
        cntrlenable);
      break;
#if 0
    case VFE_MODULE_CLFILTER:
      diagnostics->control_clfilter.cntrlenable = cntrlenable;
      vfe_clf_trigger_enable(&vfe_module->clf_mod, params, cntrlenable);
      break;
#endif
    default:
      CDBG_ERROR("%s: invalid item %d\n", __func__, item);
...
static void ez_vfe_set_reload_chromatix(vfe_ctrl_info_t *vfe_ctrl_obj)
{
...
#if 0
  vfe_bcc_reload_params(&vfe_module->bcc_mod, &vfe_ctrl_obj->vfe_params);
#endif
  vfe_asf_reload_params(mod_id, &vfe_module->asf_mod,
    &vfe_ctrl_obj->vfe_params);

  status = vfe_color_correct_reload_params(mod_id,
  ...
  status = vfe_color_conversion_reload_params(mod_id,
  ...
  status = vfe_demosaic_reload_params(&vfe_module->demosaic_mod,
    params);
  if (status != VFE_SUCCESS) {
