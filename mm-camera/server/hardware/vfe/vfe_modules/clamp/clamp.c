...
vfe_status_t vfe_clamp_init(int mod_id, void *mod_clamp, void* parm)
{
  ...
  mod->clamp_cfg_cmd.cbChanMax  = 255;
  mod->clamp_cfg_cmd.cbChanMin  = 0;
  mod->clamp_cfg_cmd.crChanMax  = 255;
  mod->clamp_cfg_cmd.crChanMin  = 0;
  mod->clamp_cfg_cmd.yChanMax   = 255;
  mod->clamp_cfg_cmd.yChanMin   = 0;
...
{
  vfe_status_t status;
  clamp_module_t *cmd = (clamp_module_t *)mod;
  vfe_params_t *vfe_params = (vfe_params_t *)vparams;

  if (VFE_SUCCESS != vfe_util_write_hw_cmd(vfe_params->camfd,
     CMD_GENERAL, (void *)&(cmd->clamp_cfg_cmd),
     sizeof(VFE_OutputClampConfigCmdType),
     VFE_CMD_OUT_CLAMP_CFG)) {
