...
#define CAMIF_MAX_OBJS 2
#define CAMIF_MAX_CLIENT_NUM 2
...
typedef struct VFE_CAMIFConfigCmdType {
  ...
  uint32_t  camif2OutputEnable : 1;
  uint32_t  camif2busEnable    : 1;
  uint32_t  framedropIRQEnable : 1;
  uint32_t  binningEnable      : 1;
  uint32_t  frameBasedEnable   : 1;
  uint32_t  /* reserved */     : 5;
  uint32_t  irqSubSamplePeriod : 5;
...
#ifdef VFE_40
  /* IRQ Subsample Pattern */
  uint32_t IRQSubsamplePattern : 32;
#endif
  /* Epoch Interrupt */
