...
{
  int i = 0;
  ISP_DBG(ISP_MOD_CLF, "%s: cutoff_1 %d cutoff_2 %d cutoff_3 %d", __func__,
    p_cmd->Cfg.cutoff_1, p_cmd->Cfg.cutoff_2,
    p_cmd->Cfg.cutoff_3);
  ISP_DBG(ISP_MOD_CLF, "%s: mult_neg %d mult_pos %d", __func__,
...
{
  int rc = TRUE;
  ISP_DBG(ISP_MOD_CLF, "%s: v_coeff0 %d v_coeff1 %d", __func__, p_cmd->chroma_coeff.v_coeff0,
     p_cmd->chroma_coeff.v_coeff1);

  ISP_DBG(ISP_MOD_CLF, "%s: h_coeff0 %d h_coeff1 %d h_coeff2 %d h_coeff3 %d", __func__,
...
{
  ISP_DBG(ISP_MOD_CLF, "%s: colorconv_enable %d pipe_flush_cnt %d", __func__,
   p_cmd->clf_cfg.colorconv_enable,
   p_cmd->clf_cfg.pipe_flush_cnt);
  ISP_DBG(ISP_MOD_CLF, "%s: pipe_flush_ovd %d flush_halt_ovd %d", __func__,
...
static int clf_config(isp_clf_mod_t *mod, isp_hw_pix_setting_params_t *in_params, uint32_t in_param_size)
{
  ...
  if (is_burst) {
    ...
    ISP_DBG(ISP_MOD_CLF, "CLF Burst shot config");
  ...
  isp_pix_trigger_update_input_t *in_params, uint32_t in_param_size)
{
  ...
  if (mod->lf_update && !mod->lf_enable) {
    ISP_DBG(ISP_MOD_CLF, "%s: update not required", __func__);

    util_clf_set_luma_params(mod, &mod->clf_params.lf_param);
    ISP_DBG(ISP_MOD_CLF, "CLF update");
  ...
  void *out_params, uint32_t out_param_size)
{
  ...
  case ISP_HW_MOD_GET_VFE_DIAG_INFO_USER: {
    ...
    ISP_DBG(ISP_MOD_CLF, "%s: Populating vfe_diag data", __func__);
  }
    break;
  default:
    rc = -1;
...
isp_ops_t *clf32_open(uint32_t version)
{
  ...
  ISP_DBG(ISP_MOD_CLF, "%s: E", __func__);
