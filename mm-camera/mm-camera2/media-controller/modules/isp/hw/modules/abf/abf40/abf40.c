  ...
  isp_hw_pix_setting_params_t *pix_setting, uint32_t in_param_size)
{
  ...
  mod->abf2_parms.data = chromatix_ABF2->abf2_config_normal_light;
  abf_set_cmd_params1(&mod->RegCmd, &mod->abf2_parms.data);
  ...
  uint32_t in_param_size)
{
  ...
  case TRIGGER_LOWLIGHT: {
    if (F_EQUAL(trigger_ratio.ratio, 0.0)) {
      memcpy(&abf2_parms->data, abf2_lowlight,
        sizeof(chromatix_adaptive_bayer_filter_data_type2));
    } else if (F_EQUAL(trigger_ratio.ratio, 1.0)) {
      memcpy(&abf2_parms->data, abf2_normal,
        sizeof(chromatix_adaptive_bayer_filter_data_type2));
    } else {
      abf_interpolate(abf2_normal, abf2_lowlight,
        abf2_parms, trigger_ratio.ratio);
    }
  }
    break;
  ...
  case TRIGGER_OUTDOOR: {
    if (F_EQUAL(trigger_ratio.ratio, 0.0)) {
      memcpy(&abf2_parms->data, abf2_outdoor,
        sizeof(chromatix_adaptive_bayer_filter_data_type2));
    } else if (F_EQUAL(trigger_ratio.ratio, 1.0)) {
      memcpy(&abf2_parms->data, abf2_normal,
        sizeof(chromatix_adaptive_bayer_filter_data_type2));
    } else {
      abf_interpolate(abf2_normal, abf2_outdoor,
        abf2_parms, trigger_ratio.ratio);
    }
  }
    break;
