...
static struct v4l2_rect prev_rect;
...
void overlay_set_params(struct mdp_blit_req *e)
{
  ...
  if (result < 0) {
    CDBG_ERROR("ERROR: MSMFB_OVERLAY_SET failed %d!\n", result);
  }
...
#endif
  if (fb_fd < 0) {
    CDBG("cannot open framebuffer %s or %s file node\n",
      ANDROID_FB0, LE_FB0);
    goto fail1;
  }

  if (ioctl(fb_fd, FBIOGET_VSCREENINFO, &vinfo) < 0) {
    CDBG("cannot retrieve vscreenInfo!\n");
    goto fail;
  }

  if (ioctl(fb_fd, FBIOGET_FSCREENINFO, &finfo) < 0) {
    CDBG("can't retrieve fscreenInfo!\n");
    goto fail;
  }
...
int v4l2_render(int frame_fd, struct v4l2_buffer *vb, struct v4l2_crop *crop)
{
  ...
  if (prev_rect.left != (__s32)e->src_rect.x ||
    prev_rect.top != (__s32)e->src_rect.y ||
    prev_rect.width != (__s32)e->src_rect.w ||
    prev_rect.height != (__s32)e->src_rect.h) {
    overlay_set_params(e);
    prev_rect.left = e->src_rect.x;
    prev_rect.top = e->src_rect.y;
    prev_rect.width = e->src_rect.w;
    prev_rect.height = e->src_rect.h;
  }
