...
int take_raw_snapshot()
{
  int rc = 0;
  CDBG("%s:BEGIN\n", __func__);
  if(0 != (rc = mm_app_take_raw_picture(0))) {
...
int main(int argc, char **argv)
{
  ...
  while ((c = getopt(argc, argv, "mouh")) != -1) {
  ...
  if(run_tc) {
    printf("%s: BEGIN: running unit test engine only\n", __func__);
    //rc = mm_app_unit_test();
    printf("%s: END: running unit test engine. EXIT(%d)!!!\n", __func__, rc);
    exit(rc);
  }

  if(MM_CAMERA_OK != (rc = mm_app_load_hal())) {
  ...
  if(MM_CAMERA_OK != (rc = mm_app_open(0, MM_CAMERA_OP_MODE_NOTUSED))) {
    CDBG_ERROR("%s:mm_app_open() err=%d\n",__func__, rc);
    exit(-2);
  }

  mm_camera_app_obj_t *pme = mm_app_get_cam_obj(0);
#ifdef USE_ION
...
static int submain()
{
  ...
  rc = system_dimension_set(0);
  if( 0 != (rc = mm_app_start_preview(0))) {
    CDBG("%s: mm_app_start_preview() err=%d\n", __func__, rc);
    return 0;
  }
    ...
    switch(action_id) {
     ...
     //   }
      ...
      case ACTION_START_RECORDING:
        ...
        if (mm_app_start_video(0) < 0)
          goto ERROR;
        break;
      case ACTION_STOP_RECORDING:
        CDBG("Stop recording action\n");
        if (mm_app_stop_video(0) < 0)
          goto ERROR;
        break;
