...
--------   ---     ----------------------------------------------------------
05/14/09   nrn     Added fix for resolving the call forwarding issue for 
                   allbearerservices
...
/* Supplimentary service request information */
typedef union qcril_cm_ss_sups_req_parm_u
{
...
} qcril_cm_ss_sups_req_parm_u_type;
...
#define QCRIL_CM_SS_ALPHA_DCS_8_BIT 0x94
#define QCRIL_CM_SS_ALPHA_DCS_UCS2  0x98
...
RIL_SsServiceType  qcril_cm_ss_get_service_type( int ss_code );

uint32 qcril_cm_ss_get_service_class( bsg_code_type bsg_type, int bsg_code );
...
void qcril_cm_ss_set_bsg_info( int service_class, basic_service_s_type *bsg_info );

uint8 qcril_cm_ss_ascii_to_utf8( char* ussd_ascii_str,  char* ussd_utf_str, uint8 ussd_ascii_len );
...
uint16 qcril_cm_ss_convert_ussd_string_to_utf8( byte uss_data_coding_scheme, byte uss_data_len, byte *uss_data, char *utf8_buf );
