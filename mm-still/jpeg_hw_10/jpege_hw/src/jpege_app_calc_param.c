...
static int scale_quant_table (jpege_quantTable * dst_quant_tbl,
...
** ======================================================================= */
/*===========================================================================
FUNCTION        jpege_app_calc_param

DESCRIPTION     configure jpege parameter:
                1) restart interval
                2) disable customized huffman tables
                3) luma and chroma quant tables
                4) jpege file size control
            ...
            uint32_t quality_factor)
{
  ...
  for (i = 0; i < 64; i++) {
     ...
     * more conservative.
     */
    dst_quant_tbl->qtbl[i] =
      (uint8_t) (((double) src_quant_tbl[i] / scale_factor) +
           0.5);
