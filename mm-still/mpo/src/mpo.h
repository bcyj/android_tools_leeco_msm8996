...
/* MPO Info (opaque definition) */
struct mpo_info_t;
typedef struct mpo_info_t *mpo_info_obj_t;
...
#define ATTRIBUTE(a) \
  (a.dependent) | (a.representative) | (a.format) | (a.type)
 ...
 *              place during the call. One should always call mpo_destroy to
 *              clean up the MPO Info object after use.
 * Input parameters:
 *   p_obj - The pointer to the MPO Info object to be initialized.
 *
 * Return values:
 *     JPEGERR_SUCCESS
 *     JPEGERR_ENULLPTR
 ...
 * Notes: none
 *****************************************************************************/
int mpo_set_index_tag(
    mpo_info_obj_t     obj,
    exif_tag_id_t      tag_id,
    exif_tag_entry_t  *p_entry);

/******************************************************************************
 * Function: mpo_set_attribute_tag
 * Description: Inserts or modifies an mpo attribute tag to the mpo Info object.
                Typical use is to call this function multiple times - to insert
                all the desired mpo Tags individually to the mpo Info object and
 *              then pass the info object to the Jpeg Encoder object so
 *              the inserted tags would be emitted as tags in the mpo header.
 * Input parameters:
 *   obj       - The mpo Info object where the tag would be inserted to or
 ...
 * Notes: none
 *****************************************************************************/
int mpo_set_attribute_tag(
    mpo_info_obj_t     obj,
    exif_tag_id_t      tag_id,
    exif_tag_entry_t  *p_entry,
    uint32_t           src_id);
/******************************************************************************
 * Function: mpo_get_tag
 * Description: Queries the stored MPO tags in the MPO Info object. Typical
 ...
 *              mpo tags are owned by the MPO Info object and they will be
 *              released when mpo_destroy is called on the MPO Info object.
 ...
 * Input parameters:
 *   obj       - The MPO Info object where the MPO tags should be obtained.
 ...
 * Notes: none
 *****************************************************************************/
int mpo_get_tag(mpo_info_obj_t     obj,
                exif_tag_id_t      tag_id,
                exif_tag_entry_t  *p_entry);

/******************************************************************************
 * Function: mpo_delete_index_tag
 * Description: Delete the stored Mpo index tags in the Mpo Info object. Typical
 *              use is to obtain an Mpo Info object from the Jpeg Decoder
 *              object which parsed an Mpo header then make multiple calls
 *              to this function to individually obtain each tag present in
 *              the header. Note that all memory allocations made to hold the
 ...
 * Notes: none
 *****************************************************************************/
int mpo_delete_index_tag(
    mpo_info_obj_t    obj,
    exif_tag_id_t     tag_id);

/******************************************************************************
 * Function: mpo_delete_attribute_tag
 * Description: Delete the stored Mpo attribute tags in the Mpo Info object.
                Typical use is to obtain an Mpo Info object from Jpeg Decoder
 *              object which parsed an Mpo header then make multiple calls
 *              to this function to individually obtain each tag present in
 *              the header. Note that all memory allocations made to hold the
 *              exif tags are owned by the Mpo Info object and they will be
...
/******************************************************************************
 * Function: mpo_list_tag_id
 * Description: List all the tags that are present in the mpo info object.
 * Input parameters:
 *   obj            - The MPO Info object where the MPO tags should be obtained.
 *   p_tag_ids      - An array to hold the MPO Tag IDs to be retrieved. It should
 ...
 * Notes: none
 *****************************************************************************/
int mpo_list_tagid(mpo_info_obj_t     obj,
                   exif_tag_id_t     *p_tag_ids,
                   uint32_t           len,
                   uint32_t          *p_len_required);

/******************************************************************************
 * Function: mpo_destroy
 ...
 *              up an 'mpo_init'-ed MPO Info object.
 * Input parameters:
 ...
 *              place during the call. One should always call
                mpo_attribute_destroy to clean up the MPO Attribute
                object after use.
 * Input parameters:
 *              p_mpo_attribute_obj - The pointer to the MPO Attribute object
                to be initialized.
 *
 * Return values:
 *     JPEGERR_SUCCESS
