...
AthBtFilter_Attach(ATH_BT_FILTER_INSTANCE *pInstance, A_UINT32 flags)
{
    ...
    do {
        ...
        status = A_MUTEX_INIT(&pInfo->BTEventLock);
        if (A_FAILED(status)) {
            A_ERR("[%s] Failed to initialize BTEventLock\n",
                  __FUNCTION__);
            break;
        }
...
AthBtFilter_Detach(ATH_BT_FILTER_INSTANCE *pInstance)
{
    ...
    A_MUTEX_DEINIT(&pInfo->BTEventLock);
...
#define A2DP_CONFIG_A2DP_IS_HIGH_PRI          ( 1 << 3)
 ...
 //               pScoGenericConfig->scoFlags |= WMI_SCO_CONFIG_FLAG_ALLOW_OPTIMIZATION;
        ...
        if (pActionMsg->IndicationForControlAction == ATH_BT_A2DP) {
            ...
            if (pA2dpGenericConfig->a2dpFlags & A2DP_CONFIG_EDR_CAPABLE) { // EDR
                if (!(pA2dpGenericConfig->a2dpFlags & A2DP_CONFIG_IS_COLOCATED_IS_MASTER)) {
                    pA2dpPspollConfig->a2dpMinBurstCnt = 8;
                }
            } else { // BDR
...
/* record the coex bitmap that turned off in BT_INQUIRY ON,
 * and only restore the corresponding coex in BT_INQUIRY OFF
 */
A_UINT32 InquiryStart_bitmap = 0xffffffff;
static void DoBtStateAction(ATHBT_FILTER_INFO *pInfo, ATHBT_STATE_INDICATION Indication, ATHBT_STATE State)
{
    ...
    if ((Indication == ATH_BT_INQUIRY) && (State == STATE_OFF)) {
        SyncBTState(pInfo);
        InquiryStart_bitmap = 0xffffffff;
    }
...
SyncBTState(ATHBT_FILTER_INFO *pInfo)
{
         ...
         stateIndication++)
    {
        if ((stateBitMap & (1 << stateIndication))
             && (InquiryStart_bitmap & (1 << stateIndication))) {
            /* this state is ON */
