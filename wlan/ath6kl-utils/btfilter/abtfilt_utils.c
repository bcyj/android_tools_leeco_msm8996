...
static const char athId[] __attribute__ ((unused)) = "$Id: //depot/sw/branches/olca/host/tools/athbtfilter/bluez/abtfilt_utils.c#9 $";
...
A_DUMP_BUFFER(A_UCHAR *buffer, int length, char *pDescription)
{
    ...
    offset = 0;
    for(i = 0; i < length; i++) {
        snprintf(stream + offset, sizeof(stream) - offset, "%2.2X ", buffer[i]);
        count ++;
        offset += 3;
