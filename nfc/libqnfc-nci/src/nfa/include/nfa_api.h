...
#define NFA_DM_NFCC_INFO                8   /* NFCC information                 */
#define NFA_DM_GET_ROUTING_EVT          9   /* RSP and NTF to NFA_GetRouting    */
/*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
#define NFA_DM_POLL_ACTIVE_ERR_EVT     10   /* Poll active error case           */
...
/*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
/* Poll active error case for JNI        */
#define NFA_POLL_ACTIVE_ERR_STATE       ePOLL_ACTIVE_ERR_STATE
...
/* Data for NFA_DM_GET_ROUTING_EVT - the RSP+NTF for GET_ROUTING_CMD*/
typedef tNFC_GET_ROUTING_REVT tNFA_GET_ROUTING;
...
/*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
/* Poll active error case           */
typedef struct
{
    UINT8 evt;
}tNFA_POLL_ACTIVE_ERR;

/*nfcc info data struct used to forward information to nfcservice*/
typedef struct
{
    UINT16          len;
    UINT8           *value;
...
typedef union
{
    ...
    tNFA_GET_ROUTING        get_routing;    /* NFA_DM_GET_ROUTING_EVT - the RSP for GET_ROUTING_CMD*/
    ...
    tNFA_DM_NFCC_INFO       nfcc_info;      /* nfcc info to nfcc service*/
/*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
    tNFA_POLL_ACTIVE_ERR    error_evt;      /* Poll active error case   */
} tNFA_DM_CBACK_DATA;
...
#define NFA_DEACTIVATE_TYPE_7816_SELECT 0xFE
#define NFA_DEACTIVATE_TYPE_T3T_INTF  0xFD
...
typedef enum {
    POLL = 0x00,
    LISTEN = 0x01
} tNFA_BERT_POLL_LISTEN;

typedef struct {
    UINT8 errorThreshold; /* Max bit errors to pass the BERT test */
} tNFA_BERT_COMMAND_RX;

typedef struct {
    UINT32 interPacketTime; /* Time in microseconds between each packet of the BERT sequence */
    UINT32 nTotalDuration; /* Total duration in TODO: unit */
} tNFA_BERT_COMMAND_TX;

typedef struct {
    tNFA_BERT_COMMAND_TX tx;
    tNFA_BERT_COMMAND_RX rx;
} tNFA_BERT_COMMAND_LOOPBACK;

typedef struct {
    tNFA_BERT_OP cmd;
    tNFA_BERT_MODE mode;
    tNFA_BERT_POLL_LISTEN  pollListen;
    tNFA_BERT_PATTERN pattern;
    tNFC_RF_TECH technology;
    tNFC_BIT_RATE bitTx;
    tNFC_BIT_RATE bitRx;
    UINT8 sofThreshold; /* Minimum SOF detected for test to pass. */
    UINT16 bitCount;
    union {
        tNFA_BERT_COMMAND_TX tx;
        tNFA_BERT_COMMAND_RX rx;
        tNFA_BERT_COMMAND_LOOPBACK loopback;
    };
} tNFA_BERT_COMMAND;
...
**
*******************************************************************************/
NFC_API extern tNFA_STATUS NFA_DTA_Set_Pattern_Number (int pattern);
/*******************************************************************************
**
...
**                  Moroever it checks whether the NFA thread should
...
**
*******************************************************************************/
extern BOOLEAN NFA_DTA_WritePatternToFile(int pattern);
...
**
*******************************************************************************/
extern long int NFA_DTA_ReadPatternFromFile(void);
#endif // </DTA>
...
**
** Description      Get the routing parameters from NFCC. The result is
...
**
*******************************************************************************/
NFC_API extern tNFA_STATUS NFA_GetRouting(void);
