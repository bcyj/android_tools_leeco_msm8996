...
void nfa_rw_handle_sleep_wakeup_rsp (tNFC_STATUS status)
{
                ...
                &&(nfa_rw_cb.pa_sel_res == NFC_SEL_RES_NFC_FORUM_T2T)  )
    {
        ...
        if (status == NFC_STATUS_OK)
        {
            ...
            memset(&tag_params, 0x00, sizeof(tNFA_TAG_PARAMS));
            if (nfa_rw_cb.nfcid1_len != 0)
            {
                memcpy (tag_params.t2t.uid, nfa_rw_cb.nfcid1, nfa_rw_cb.nfcid1_len);
                nfa_rw_cb.nfcid1_len = 0;
                memset(nfa_rw_cb.nfcid1, 0x00, sizeof(nfa_rw_cb.nfcid1));
            }
...
static void nfa_rw_cback (tRW_EVENT event, tRW_DATA *p_rw_data)
{
    ...
    if (nfa_rw_cb.p_pending_msg)
    {
        ...
        {
            tNFA_RW_MSG     *p_data = nfa_rw_cb.p_pending_msg;
            UINT16     presence_check_start_delay = 0;

            if (p_data->op_req.params.send_raw_frame.p_data != NULL)
            {
...
void nfa_rw_presence_check (tNFA_RW_MSG *p_data)
{
    ...
    case NFC_PROTOCOL_ISO_DEP:     /* ISODEP/4A,4B- NFC-A or NFC-B */
        ...
        case NFA_RW_PRES_CHK_NACK:
            option = RW_T4T_CHK_NACK;
            break;
        default:
...
BOOLEAN nfa_rw_activate_ntf(tNFA_RW_MSG *p_data)
{
        ...
        &&(nfa_rw_cb.pa_sel_res == NFC_SEL_RES_NFC_FORUM_T2T)  )
    {
        ...
        memset(nfa_rw_cb.nfcid1, 0x00, NCI_NFCID1_MAX_LEN);
        nfa_rw_cb.nfcid1_len = p_activate_params->rf_tech_param.param.pa.nfcid1_len;
        memcpy(nfa_rw_cb.nfcid1, p_activate_params->rf_tech_param.param.pa.nfcid1, p_activate_params->rf_tech_param.param.pa.nfcid1_len);
        return TRUE;
    }
...
BOOLEAN nfa_rw_handle_op_req (tNFA_RW_MSG *p_data)
{
    ...
    {
        freebuf = nfa_rw_op_req_while_busy(p_data);

        if (freebuf == FALSE)
        {
            NFA_TRACE_DEBUG0("pending cmd");
