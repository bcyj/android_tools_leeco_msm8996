...
BOOLEAN nfa_p2p_reg_client (tNFA_P2P_MSG *p_msg)
{
...
#ifdef DTA // <DTA>
    /* local_sap = NULL in case of non DTA client */
    if (p_msg->api_reg_client.local_sap == NULL)
    {
#endif // </DTA>
    ...
    {
        local_sap = LLCP_RegisterCLClient (p_msg->api_reg_client.link_type,
                                         p_msg->api_reg_client.local_sap,
                                         nfa_p2p_llcp_cback);
    }
#endif // </DTA>
...
BOOLEAN nfa_p2p_dereg (tNFA_P2P_MSG *p_msg)
{
    ...
    if (nfa_p2p_cb.is_p2p_listening)
    {
        ...
        {
            /* FIXME: This restarts discovery during disable,
             * might be needed if we have 2nd NFC switch
             * nfa_p2p_enable_listening (NFA_ID_P2P, TRUE);
             */
        }
    }
