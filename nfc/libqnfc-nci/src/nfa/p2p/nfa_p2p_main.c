        ...
        if (reason != LLCP_LINK_RF_LINK_LOSS_ERR) /* if NFC link is still up */
        {
            if (nfa_p2p_cb.is_initiator)
            {
                ...
                if(in_llcp_or_snep_dta_mode() &&
                   ((reason == LLCP_LINK_RF_PROTOCOL_ERR) || (reason == LLCP_LINK_REMOTE_INITIATED))) {
                /* FIXME: check if these reason checks are needed */
                /* in case of protocol error send deactivate to sleep */
