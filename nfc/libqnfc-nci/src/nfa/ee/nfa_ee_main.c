...
void nfa_ee_init (void)
{
    ...
    nfa_ee_cb.max_aid_route_entries = NFA_EE_NUM_AID_ROUTE_ENTRIES;
    if (GetNumValue("NUM_AID_ROUTE_ENTRIES", &xx, sizeof(xx)))
    {
        nfa_ee_cb.max_aid_route_entries = xx;
    }
...
BOOLEAN nfa_ee_evt_hdlr (BT_HDR *p_msg)
{
    ...
    case NFA_EE_EM_STATE_INIT:
        if ((p_msg->event == NFA_EE_NCI_DISC_NTF_EVT) ||
            ...
            (p_msg->event == NFA_EE_NCI_DISC_REQ_NTF_EVT))
        {
            act = TRUE;
        }
        break;
