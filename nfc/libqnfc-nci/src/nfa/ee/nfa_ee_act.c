    ...
    else
    {
        ...
        if ((len_needed + len) > NFA_EE_MAX_AID_CFG_LEN)
        {
            ...
            evt_data.status = NFA_STATUS_BUFFER_FULL;
                 ...
                 (p_cb->aid_entries < nfa_ee_cb.max_aid_route_entries))
        {
        ...
        {
            NFA_TRACE_ERROR2 ("Exceed NFA_EE_MAX_AID_ENTRIES:%d or NFCC limit:%d",
                               NFA_EE_MAX_AID_ENTRIES, nfa_ee_cb.max_aid_route_entries);
            evt_data.status = NFA_STATUS_BUFFER_FULL;
        }
    }
...
**
*******************************************************************************/
void nfa_ee_check_set_routing(UINT16 new_size, UINT8  nfcee_id,int *p_max_len, UINT8 *p, int *p_cur_offset)
{
    ...
    {
        if (NFC_SetRouting(TRUE,nfcee_id, *p, *p_cur_offset, p + 1) == NFA_STATUS_OK)
        {
            nfa_ee_cb.wait_rsp++;
        }
...
tNFA_STATUS nfa_ee_route_add_one_ecb(tNFA_EE_ECB *p_cb, int *p_max_len, BOOLEAN more, UINT8 *ps, int *p_cur_offset)
{
    ...
    nfa_ee_check_set_routing (p_cb->size_mask,p_cb->nfcee_id, p_max_len, ps, p_cur_offset);
    ...
    *ps              = num_tlv;
    ...
    if (p_cb->aid_entries)
    {
        ...
        for (xx = 0; xx < p_cb->aid_entries; xx++)
        {
            ...
            nfa_ee_check_set_routing(new_size,p_cb->nfcee_id, p_max_len, ps, p_cur_offset);
            if (*ps == 0)
            {
    ...
    if (more == FALSE)
    {
        ...
        if (nfa_ee_cb.ee_cfg_sts & NFA_EE_STS_CHANGED_ROUTING)
        {
            ...
            if (NFC_SetRouting(more, p_cb->nfcee_id,num_tlv, (UINT8)(*p_cur_offset), ps + 1) == NFA_STATUS_OK)
            {
                nfa_ee_cb.wait_rsp++;
            }
        }
        else if (nfa_ee_cb.ee_cfg_sts & NFA_EE_STS_PREV_ROUTING)
        {
            if (tlv_size == 0)
            {
                ...
                nfa_ee_cb.ee_cfg_sts       |= NFA_EE_STS_CHANGED_ROUTING;
                if (NFC_SetRouting(more, p_cb->nfcee_id, 0, 0, ps + 1 )== NFA_STATUS_OK)
                {
                    nfa_ee_cb.wait_rsp++;
                }
            }
