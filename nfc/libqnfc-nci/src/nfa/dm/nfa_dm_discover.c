                                            ...
                                            UINT8 max_params)
{
    ...
    if ((dm_disc_mask & NFA_DM_DISC_MASK_P_ISO15693) && (nfa_dm_cb.nfcc_version >=  NFCC_CHIP_VERSION_3))
    {
...
#ifdef NFC_FORUM_T1_T2_TAG_EMULATION_SUPPORTED
/* if UICC supports T2T then NFC-DEP/ISO-DEP could be supported in DH */
    ...
    */
    ...
    enable = 0;
    if (GetNumValue("ADD_LISTEN_A_FOR_P2P_GIVEN_ONLY_LISTEN_B", &enable, sizeof(enable)))
    {
        if ((enable != 0) && (nfa_dm_cb.disc_cb.listen_RT[NFA_DM_DISC_LRT_NFC_B] != NFA_DM_DISC_HOST_ID_DH))
        {
            tNFA_EE_ECB *p_ee_ecb;
            p_ee_ecb = nfa_ee_find_ecb (nfa_dm_cb.disc_cb.listen_RT[NFA_DM_DISC_LRT_NFC_B]);
            if (p_ee_ecb)
            {
                /* add ISO-DEP if listen B is present and uicc doesn't support listen A */
                if((p_ee_ecb->lb_protocol == NFA_PROTOCOL_ISO_DEP) &&
                   (p_ee_ecb->la_protocol == 0) &&
                   (sens_info & NCI_PARAM_SEL_INFO_NFCDEP) &&
                   (nfa_dm_cb.orange_route))
                {
                    sens_info |= NCI_PARAM_SEL_INFO_ISODEP;
                }
            }
    ...
    /* for Listen F */
#ifdef SUPPORT_NFC_DEP_ONLY_LMRT_TECH_F_TO_DH
...
void nfa_dm_set_default_listen_mode_routing_table(void)
{
    ...
    if ( GetStrValue ("DEFAULT_LISTEN_MODE_ROUTING", (char*)default_listen_mode_routing_table, sizeof(default_listen_mode_routing_table ) ) )
    {
        ...
        if(tlv_size <= sizeof(default_listen_mode_routing_table ) - 3)
        {
            ...
            return;
        } else NFA_TRACE_DEBUG0("NFA nfa_dm_set_default_listen_mode_routing_table: routing table too long");
    } else NFA_TRACE_DEBUG0 ("No default listen mode routing table found in conf file...Sending Default from code");

    UINT8 backup_default_listen_mode_routing_table[10]= {0x01,0x03,0x00,0x01,0x04,0x01,0x03,0x00,0x01,0x05};
    num_tlv = 2;
    tlv_size = 10;
    NFC_SetRouting(more, nfceeid, num_tlv, tlv_size, backup_default_listen_mode_routing_table);
...
void nfa_dm_start_rf_discover (void)
{
    ...
    else
    {
...
#ifdef T3T_EMU_ON_DH
                /* allow NFC-DEP on DH */
                if (nfa_dm_cb.disc_cb.entry[xx].host_id == NFA_DM_DISC_HOST_ID_DH)
                {
                    listen_mask |= nfa_dm_cb.disc_cb.entry[xx].requested_disc_mask
                                   & ( NFA_DM_DISC_MASK_LF_NFC_DEP
                                      |NFA_DM_DISC_MASK_LFA_NFC_DEP );
...
#ifdef DTA // <DTA>
        /* Disable active P2P mode if DTA pattern number if 0xFFFF */
        if (in_llcp_or_snep_dta_mode())
        {
            NFA_TRACE_DEBUG0 ("[DTA] dta pattern number is FFFF, disable active P2P mode");
        ...
        {
#ifdef DTA // <DTA>
            if((in_llcp_or_snep_dta_mode()) || !(in_dta_mode()) ) {
#endif // </DTA>
    ...
    {
        /*
         * TODO: First we need to check if type is already included at the disc params
         * before adding it
         */
        ...
        if(listenmask != 0)
        {
            if( GetNumValue("P2P_LISTEN_TECH_MASK", &p2p_listen_mask, sizeof(p2p_listen_mask)) )
            {
                ...
                {
                                    if ((num_params +1) < NFA_DM_MAX_DISC_PARAMS) {
                    ...
                    num_params +=1 ;
                                    ...
                                    {
                                        NFA_TRACE_ERROR0("nfa_dm_start_rf_discover: disc_params[] no entries left");
                                    }
                }
                else if(p2p_listen_mask == 0x04)
                {
                                    if ((num_params +2) < NFA_DM_MAX_DISC_PARAMS) {
            ...
            if(p2p_listen_mask == 0x00)
            {
                ...
                {
                                    if ((num_params +3) < NFA_DM_MAX_DISC_PARAMS) {
...
static tNFA_STATUS nfa_dm_disc_notify_activation (tNFC_DISCOVER *p_data)
{
    ...
    if ((p_data->activate.intf_param.type != NFC_INTERFACE_EE_DIRECT_RF) &&
        ((p_data->activate.rf_tech_param.mode & 0x80) == 0x00) &&
        (nfa_dm_cb.req_screen_off != 0xFF))
    {
        tNFC_RESPONSE   evt_data;
        nfa_dm_cb.req_screen_off = 0xFF;
        NFA_TRACE_DEBUG0 ("nfa_dm_disc_notify_activation:pending screen cmd");
        nfa_dm_cb.resend_screen = TRUE;
        evt_data.status = (tNFC_STATUS) NFC_STATUS_OK;
        evt_data.error.state = POLL_ACTIVE_ERR_REQ_DEACT;
        nfc_main_notify_poll_active_error(&evt_data);
        return (NFA_STATUS_OK);
    }/*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
    ...
    else
    {
        ...
        else
        {
            ...
            if (nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_DISABLING)
            {
                ...
                {
                    llcp_link_deactivate(NFC_STATUS_LINK_LOSS);
                }
            }
...
void nfa_dm_disc_new_state (tNFA_DM_RF_DISC_STATE new_state)
{
    ...
    else if (new_state == NFA_DM_RFST_DISCOVERY)
    {
/*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
        if (nfa_dm_cb.resend_screen == TRUE)
        {
            tNFC_RESPONSE   evt_data;
            nfa_dm_cb.resend_screen = FALSE;
            evt_data.status             = (tNFC_STATUS) NFC_STATUS_OK;
            evt_data.error.state = POLL_ACTIVE_ERR_RESEND_UICMD;
            nfc_main_notify_poll_active_error(&evt_data);
        }
 /*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
    }
                                        ...
                                        tNFA_DM_RF_DISC_DATA *p_data)
{
    ...
    case NFA_DM_RF_DEACTIVATE_CMD:
        ...
        {
            if((nfa_dm_cb.disc_cb.disc_flags & (NFA_DM_DISC_FLAGS_W4_NTF|NFA_DM_DISC_FLAGS_W4_RSP)))
            {
                nfa_dm_cb.disc_cb.deact_pending      = TRUE;
                nfa_dm_cb.disc_cb.pending_deact_type = p_data->deactivate_type;
            }
    ...
    case NFA_DM_CORE_INTF_ERROR_NTF:
        ...
        {
            /* in case of previous deact_cmd, DH shouldn't wait for deact_NTF */
            nfa_dm_cb.disc_cb.disc_flags &= ~NFA_DM_DISC_FLAGS_W4_NTF;
            /* NCI bug, instead of deact_descovery, DH should send deact_idle */
#ifdef DTA // <DTA>
            if (in_llcp_or_snep_dta_mode())
            {
                                          ...
                                          tNFA_DM_RF_DISC_DATA     *p_data)
{
    ...
    case NFA_DM_RF_DEACTIVATE_RSP:
        ...
        break;
    case NFA_DM_RF_DEACTIVATE_NTF:
        if ((p_data->nfc_discover.deactivate.type == NFA_DEACTIVATE_TYPE_7816_SELECT) ||
            (p_data->nfc_discover.deactivate.type == NFA_DEACTIVATE_TYPE_T3T_INTF))
        {
            NFA_TRACE_DEBUG0 ("Rx deactivate NTF with 0xFE, notify and stay in listen active state");
            nfa_dm_disc_notify_deactivation (NFA_DM_RF_DEACTIVATE_NTF, &(p_data->nfc_discover));
            break;
        }
    ...
    case NFA_DM_RF_INTF_ACTIVATED_NTF:
        NFA_TRACE_DEBUG0 ("Rx activated NTF in listen active state, stay in listen active");
        if (nfa_dm_disc_notify_activation (&(p_data->nfc_discover)) == NFA_STATUS_FAILED)
        {
            NFA_TRACE_DEBUG0 ("Not matched, wait for another activated/deactivated ntf");
        }
        break;
                                       ...
                                       tNFA_INTF_TYPE    rf_interface)
{
    ...
    {
        /*
         * We use NFA_Select for P2P in DTA mode, but there is nothing to be selected on the RF
         * Notify NativeNfcManager that "selection" worked.
         * TODO: To increase fail safety, limit check for in_dta_mode() and protocol = NFCDEP and not active
         * */
        conn_evt.status = NFA_STATUS_OK;
        nfa_dm_conn_cback_event_notify (NFA_SELECT_RESULT_EVT, &conn_evt);
    }
