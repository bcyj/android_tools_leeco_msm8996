...
static void nfa_dm_nfc_response_cback (tNFC_RESPONSE_EVT event, tNFC_RESPONSE *p_data)
{
    ...
    case NFC_GET_ROUTING_REVT:                   /* Routing Table NTF */
         /* Return result of getRouting to the app */
        (*nfa_dm_cb.p_dm_cback) ( NFA_DM_GET_ROUTING_EVT, (tNFA_DM_CBACK_DATA *)&(p_data->get_routing) );
    ...
    case NFC_NFCC_INFO_REVT:
        /*send nfcc info to nfc JNI*/
        if(p_data->nfcc_info_data.len != 0)
        {
             NFA_TRACE_DEBUG1 ("p_data->nfcc_info_data->len = %d",p_data->nfcc_info_data.len);
             dm_cback_data.nfcc_info.len = p_data->nfcc_info_data.len;

             dm_cback_data.nfcc_info.value = (UINT8*)malloc(dm_cback_data.nfcc_info.len);

             if(dm_cback_data.nfcc_info.value != NULL)
             {
                 memcpy(dm_cback_data.nfcc_info.value,p_data->nfcc_info_data.value,p_data->nfcc_info_data.len);
                 if(nfa_dm_cb.p_dm_cback)
                 {
                     (*nfa_dm_cb.p_dm_cback) (NFA_DM_NFCC_INFO, &dm_cback_data);
                 }
                 /* free acquired memory*/
                 free(dm_cback_data.nfcc_info.value);
                 dm_cback_data.nfcc_info.value = NULL;
             }
             else
                 NFA_TRACE_ERROR0 ("Memory allocation failed");
        }
        else
            NFA_TRACE_ERROR0 ("NFCC info data len is 0");

        break;
/* QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
    case POLL_ACTIVE_ERR_REVT:
        if(nfa_dm_cb.p_dm_cback)
        {
            dm_cback_data.error_evt.evt = p_data->error.state;
            (*nfa_dm_cb.p_dm_cback) (NFA_DM_POLL_ACTIVE_ERR_EVT, &dm_cback_data);
        }
        break;
/* QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
    default:
...
**
*******************************************************************************/
BOOLEAN nfa_dm_store_shutdown_reason(tNFA_DM_MSG *p_data)
{
...
**
*******************************************************************************/
BOOLEAN nfa_dm_check_reset_status(tNFA_DM_MSG *p_data)
{
...
/*******************************************************************************
**
** Function        nfa_dm_dta_set_pattern
**
** Description     Process dta pattern and send set config command
**
** Returns          TRUE (message buffer to be freed by caller)
**
*******************************************************************************/
BOOLEAN nfa_dm_dta_set_pattern(tNFA_DM_MSG *p_data)
{
    tNFC_STATUS status = NFC_STATUS_OK;

    /* The payload consists of TLVs. The first byte is always the TLV count.
     * This leaves us 251 bytes of payload for the actual TLVs.
     *
     * Number of items (N) 1 Byte Number of item TLVs following
     * Item 1 Tag 1 Byte Tag for item 1 TLV (see below)
     * Item 1 Length (L1) 1 Byte Length for item 1 TLV Value
     * Item 1 Value L1 Bytes Value for item 1 TLV
     * ..........
     * Item N Tag 1 Byte Tag for item N TLV (see below)
     * Item N Length (LN) 1 Byte Length for item N TLV Value
     * Item N Value LN Bytes Value for item N TLV
     */
    #define TLV_PAYLOAD_LEN (NCI_MAX_CTRL_SIZE - NCI_MSG_HDR_SIZE - 1)
    UINT8       buff[TLV_PAYLOAD_LEN];

    memset(buff, 0x00, TLV_PAYLOAD_LEN);
    if (p_data != NULL)
    {
        if (p_data->dta_pattern.length <= TLV_PAYLOAD_LEN)
        {
            memcpy(buff, p_data->dta_pattern.p_data, p_data->dta_pattern.length);
            status = nfa_dm_check_set_config (p_data->dta_pattern.length, buff, FALSE);
        }
        else
        {
            status = NFC_STATUS_INVALID_PARAM;
        }
    }
    else
    {
        /* helps us distinguish failure points in the logs by using another failure value */
        status = NFC_STATUS_NOT_INITIALIZED;
    }

    if (status != NFC_STATUS_OK)
    {
         /* Only print status on failure */
         NFA_TRACE_DEBUG1 ("dta_set_pattern, status 0x%x", status);
    }
    return (TRUE);
}
#endif // <DTA>
...
BOOLEAN nfa_dm_disable (tNFA_DM_MSG *p_data)
{
    ...
    {
        /*
                    For two cases, MW will check to send DEACTIVATE cmd in disabling flow
                    1. RF discovery is enabled
                    2. None triggered discovery(disc_flags == 0) and received disable request
                        If MW doesn't check this case, MW cannot switch FW and NFA task to idle mode after disabling flow.
                        MW will enter timeout error handler flow. The disabling flow will take more than 3~ 4 seconds.
                        In the normal case, disabling flow takes about 1 second.
        */
        if ((nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_ENABLED)||(nfa_dm_cb.flags & NFA_DM_FLAGS_EXCL_RF_ACTIVE))
        {
...
/*******************************************************************************
**
** Function         nfa_dm_get_routing
**
** Description      Process the NFA command to get Listen Mode Routing Table
**                  entries by calling the corresponding NFC command.
...
**
*******************************************************************************/
BOOLEAN nfa_dm_get_routing(tNFA_DM_MSG *p_data)
{
  NFC_GetRouting();
