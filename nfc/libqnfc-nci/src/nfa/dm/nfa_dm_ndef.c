                                                          ...
                                                          UINT32                    payload_len)
{
    tNFA_DM_CB *p_cb = &nfa_dm_cb;
    UINT8 i;
    /* if p_type_name == NULL we were exiting the for and returning null*/
    /* This is added to avoid klockwork issues */
    if (p_type_name == NULL) {
        NFA_TRACE_DEBUG0("nfa_dm_ndef_find_next_handler: null");
        return (NULL);
    }
