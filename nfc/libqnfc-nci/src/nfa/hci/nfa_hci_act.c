...
static void nfa_hci_api_reconnect (tNFA_HCI_EVENT_DATA *p_evt_data);
...
/* To Track Shutdown reason from nfcservice*/
...
void nfa_hci_check_api_requests (void)
{
    ...
    for ( ; ; )
    {
        ...
        case NFA_HCI_API_GET_HOST_LIST_EVT:
            if(shutingdown_reason != NFC_DISABLED_FROM_UI)
            {
        ...
        case NFA_HCI_API_RECONNECT_EVT:
            nfa_hci_api_reconnect (p_evt_data);
            break;
...
/*******************************************************************************
**
** Function         nfa_hci_api_reconnect
**
** Description      action function to reconnect
**
** Returns          None
**
*******************************************************************************/
static void nfa_hci_api_reconnect (tNFA_HCI_EVENT_DATA *p_evt_data)
{
    ...
    NFA_TRACE_DEBUG2 ("nfa_hci_api_reconnect() ee_handle:0x%04x => 0x%04x",
                      nfa_hci_cb.reconnect_ee_handle, p_evt_data->reconnect.ee_handle);

    if ((nfa_hci_cb.conn_id) &&
        ((nfa_hci_cb.reconnect_ee_handle & NFA_HANDLE_MASK) == (p_evt_data->reconnect.ee_handle & NFA_HANDLE_MASK)))
    {
        NFA_TRACE_DEBUG0 ("nfa_hci_api_reconnect() already connected");
        evt_data.reconnected.status = NFA_STATUS_OK;
        nfa_hciu_send_to_app (NFA_HCI_RECONNECT_EVT, &evt_data, p_evt_data->reconnect.hci_handle);
        return;
    }

    nfa_hci_cb.reconnecting = TRUE;
    nfa_hci_cb.reconnect_ee_handle = p_evt_data->reconnect.ee_handle;
    nfa_hci_cb.reconnect_hci_handle = p_evt_data->reconnect.hci_handle;
        ...
        case NFA_HCI_ANY_GET_PARAMETER:
...
#ifndef TODO // TODO:
                    if (host_id == NFA_HCI_HOST_ID_ESE)
                    {
                        NFA_TRACE_DEBUG1 ("Found Host ID=0x%x", host_id);
                        host_id = 0x02;
                    }
#endif
            ...
            else if (nfa_hci_cb.param_in_use == NFA_HCI_SESSION_IDENTITY_INDEX)
            {
                ...
                if ((!memcmp ((UINT8 *) nfa_hci_cb.cfg.admin_gate.session_id, p_data, NFA_HCI_SESSION_ID_LEN)) ||
                   (nfa_hci_cb.reconnecting)) // do not clear all pipe during reconnecting
                {
...
void nfa_hci_handle_admin_gate_evt (UINT8 *p_data)
{
    ...
    if (nfa_hci_cb.reconnecting)
    {
        NFA_TRACE_DEBUG0 ("nfa_hci_handle_admin_gate_evt - ignore HOT PLUG EVT during reconnecting");
        return;
    }
...
static void nfa_hci_handle_generic_gate_rsp (UINT8 *p_data, UINT8 data_len, tNFA_HCI_DYN_GATE *p_gate, tNFA_HCI_DYN_PIPE *p_pipe)
{
    ...
    if (nfa_hci_cb.inst != NFA_HCI_ANY_OK)
    {
        ...
        if (nfa_hci_cb.pipe_in_use == 0x72)
        {
            evt_data.rcvd_evt.pipe      = 0x72;
            evt_data.rcvd_evt.evt_code  = 0x10;
            evt_data.rcvd_evt.evt_len   = 0;
            evt_data.rcvd_evt.status    = status;
...
static void nfa_hci_handle_generic_gate_evt (UINT8 *p_data, UINT16 data_len, tNFA_HCI_DYN_GATE *p_gate, tNFA_HCI_DYN_PIPE *p_pipe)
{
    ...
    if (  (evt_data.rcvd_evt.pipe == 0x72)
        &&(evt_data.rcvd_evt.evt_code == 0x11))
    {
        /* notify NFA_HCI_EVENT_RCVD_EVT/EVT_WTX_REQUEST to the application */
        nfa_hciu_send_to_app (NFA_HCI_EVENT_RCVD_EVT, &evt_data, p_gate->gate_owner);
        return;
    }
