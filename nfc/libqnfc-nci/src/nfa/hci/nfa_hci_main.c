...
void nfa_hci_ee_info_cback (tNFA_EE_DISC_STS status)
{
    ...
    case NFA_EE_DISC_STS_REQ:
        ...
        break;
#endif /* NCI_VIRTUAL_NFCEE_SUPPORTED */
    }
}
...
{
    INT32 nfa_ee_discv_timeout_val;
    ...
    if (nfa_hci_cb.w4_hci_netwk_init)
    {
        if (nfa_hci_cb.hci_state == NFA_HCI_STATE_STARTUP)
        {
            ...
            {
                /* don't need to wait for HCI/SWP initialization, use minimum timeout */
                nfa_sys_start_timer (&nfa_hci_cb.timer, NFA_HCI_RSP_TIMEOUT_EVT, 10);
            }
            else
            {
                if (GetNumValue("NFA_EE_DISCV_TIMEOUT_VAL", &nfa_ee_discv_timeout_val, sizeof(nfa_ee_discv_timeout_val)))
                {
                    NFA_TRACE_DEBUG1 ("nfa_hci_dh_startup_complete (): NFA_EE_DISCV_TIMEOUT_VAL = %dms", nfa_ee_discv_timeout_val);
                }
                else
                {
                    nfa_ee_discv_timeout_val = NFA_EE_DISCV_TIMEOUT_VAL;
                }
...
void nfa_hci_startup_complete (tNFA_STATUS status)
{
    ...
    if (nfa_hci_cb.reconnecting)
    {
        ...
        // reconnecting to NFCEE is done
        evt_data.reconnected.status = status;
        nfa_hciu_send_to_app (NFA_HCI_RECONNECT_EVT, &evt_data, nfa_hci_cb.reconnect_hci_handle);

        nfa_hci_cb.reconnecting = FALSE;
        return;
    }
    ...
    if (nfa_hci_cb.nv_read_cmplt && nfa_hci_cb.ee_disc_cmplt && (nfa_hci_cb.conn_id == 0))
    {
        ...
        while ((count < num_nfcee) && (!found))
        {
            ...
            if(ee_info[count].ee_interface[0] == NFA_EE_INTERFACE_HCI_ACCESS)
            {
                ...
                {
                    // store current connected NFCEE not to reconnect later
                    nfa_hci_cb.reconnect_ee_handle = target_handle;
...
static void nfa_hci_conn_cback (UINT8 conn_id, tNFC_CONN_EVT event, tNFC_CONN *p_data)
{
    ...
    if (event == NFC_CONN_CREATE_CEVT)
    {
        ...
        if (nfa_hci_cb.reconnecting)
        {
            NFA_TRACE_DEBUG0 ("If HCI reconnecting, keep pipe open");
    ...
    else if (event == NFC_CONN_CLOSE_CEVT)
    {
        ...
        if (nfa_hci_cb.reconnecting)
        {
            ...
            if (NFC_ConnCreate (NCI_DEST_TYPE_NFCEE,
                                nfa_hci_cb.reconnect_ee_handle,
            ...
            {
                nfa_hci_cb.hci_state = NFA_HCI_STATE_DISABLED;
                NFA_TRACE_ERROR0 ("Failed to Create Logical connection for reconnection ");
                nfa_hci_startup_complete (NFA_STATUS_FAILED);
            }
        }
        else
