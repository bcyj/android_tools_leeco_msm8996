...
/*******************************************************************************
**
** Function         NFA_HciReconnect
**
** Description      This function will reconnect logical connection to NFCEE which
**                  is specified by ee_handle.
**                  When reconnection and intialization are completed
**                  the app will be notified with NFA_HCI_RECONNECT_EVT.
**
** Returns          NFA_STATUS_OK if successfully initiated
**                  NFA_STATUS_FAILED otherwise
**
*******************************************************************************/
tNFA_STATUS NFA_HciReconnect (tNFA_HANDLE hci_handle, tNFA_HANDLE ee_handle)
{
    tNFA_HCI_API_RECONNECT *p_msg;

    if ((NFA_HANDLE_GROUP_MASK & hci_handle) != NFA_HANDLE_GROUP_HCI)
    {
        NFA_TRACE_API1 ("NFA_HciReconnect (): Invalid hci_handle:0x%04x", hci_handle);
        return (NFA_STATUS_FAILED);
    }

    if ((NFA_HANDLE_GROUP_MASK & ee_handle) != NFA_HANDLE_GROUP_EE)
    {
        NFA_TRACE_API1 ("NFA_HciReconnect (): Invalid ee_handle:0x%04x", ee_handle);
        return (NFA_STATUS_FAILED);
    }

    NFA_TRACE_API2 ("NFA_HciReconnect (): hci_handle:0x%04x, ee_handle:0x%04x", hci_handle, ee_handle);

    if (  (nfa_hci_cb.hci_state != NFA_HCI_STATE_DISABLED)
        &&((p_msg = (tNFA_HCI_API_RECONNECT *) GKI_getbuf (sizeof (tNFA_HCI_API_RECONNECT))) != NULL))
    {
        p_msg->hdr.event  = NFA_HCI_API_RECONNECT_EVT;
        p_msg->hci_handle = hci_handle;
        p_msg->ee_handle  = ee_handle;
