                              ...
                              UINT8 *p_gen_bytes_len)
{
    ...
    *p_wt = llcp_cb.lcb.local_wt;
...
#ifdef DTA // <DTA> In LLCP/SNEP DTA mode do not send MIUX info
    ...
    UINT8_TO_BE_STREAM (p, LLCP_WKS_TYPE);
    UINT8_TO_BE_STREAM (p, LLCP_WKS_LEN);
#ifdef DTA // <DTA> In LLCP/SNEP DTA mode use wks 0x03 (do not advertise SNEP)
    if (!in_llcp_or_snep_dta_mode()) {
#endif // </DTA>
      UINT16_TO_BE_STREAM (p, llcp_cb.lcb.wks);
    }
...
#ifdef DTA // <DTA> In LLCP/SNEP DTA mode do not set OPT
    if (!in_llcp_or_snep_dta_mode()) {
#endif // </DTA>
                           ...
                           tLLCP_APP_CBACK *p_app_cback)
{
    ...
    if (reg_sap == LLCP_INVALID_SAP)
    {
        ...
        for (sap = 0; sap < LLCP_MAX_SERVER; sap++)
        {
            ...
            {
#ifdef DTA // <DTA>
                /* don't give a test sap */
                if (((LLCP_LOWER_BOUND_SDP_SAP + sap) != LLCP_ECHO_CO_SAP) && ((LLCP_LOWER_BOUND_SDP_SAP + sap) != LLCP_ECHO_CL_SAP)) {
#endif // </DTA>
                           ...
                           tLLCP_APP_CBACK *p_app_cback)
{
    ...
    for (sap = 0; sap < LLCP_MAX_CLIENT; sap++)
    {
        ...
        {
#ifdef DTA // <DTA>
            /* reserve this sap for CL*/
            if ((LLCP_LOWER_BOUND_LOCAL_SAP +sap) != LLCP_ECHO_CL_C_SAP) {
#endif // </DTA>
...
/*******************************************************************************
**
** Function         LLCP_RegisterCLClient
**
** Description      Register client and callback function
**
**                  link_type : LLCP_LINK_TYPE_LOGICAL_DATA_LINK
...
**
*******************************************************************************/
UINT8 LLCP_RegisterCLClient (UINT8           link_type,
                             UINT8           sap,
                             tLLCP_APP_CBACK *p_app_cback)
{
    UINT8 reg_sap = LLCP_INVALID_SAP;
    ...
    else if (  (sap >= (LLCP_LOWER_BOUND_LOCAL_SAP + LLCP_MAX_CLIENT))
             ||(sap < LLCP_LOWER_BOUND_LOCAL_SAP)  )
    {
        LLCP_TRACE_ERROR1 ("LLCP_RegisterClient (): Invalid SAP (0x%x)", sap);
        return LLCP_INVALID_SAP;
    }

    p_app_cb = &llcp_cb.client_cb[sap - LLCP_LOWER_BOUND_LOCAL_SAP];
    memset (p_app_cb, 0x00, sizeof (tLLCP_APP_CB));
    reg_sap = sap;
