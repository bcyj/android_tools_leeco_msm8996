...
{
    UINT8      *p, * pp, *p_dst, send_seq, rcv_seq, error_flags;
    ...
    if ((p_dlcb)&&(p_dlcb->state == LLCP_DLC_STATE_CONNECTED))
    {
        ...
        {
            LLCP_TRACE_ERROR0("llcp_dlc_proc_i_pdu (): not able to access p_i_pdu");
            return;
        }
        ...
        pp = p_i_pdu + LLCP_PDU_HEADER_SIZE + LLCP_SEQUENCE_SIZE;

#ifdef DTA // <DTA>
        if (in_llcp_or_snep_dta_mode() && llcp_dlc_proc_i_param (pp, p_dlcb))
        {
            p_dlcb->state = LLCP_DLC_STATE_W4_REMOTE_DM;
...
BT_HDR* llcp_dlc_get_next_pdu (tLLCP_DLCB *p_dlcb)
{
        ...
        &&(llcp_dlc_is_rw_open (p_dlcb))  )
    {
        ...
        llcp_cb.total_tx_i_pdu--;
        if (p_msg == NULL) {
            LLCP_TRACE_ERROR0("Not able to retrieve msg from the queue");
            goto done_send;
        }
...
#ifdef DTA // <DTA>
BOOLEAN llcp_dlc_proc_i_param (UINT8 *p_i_payload, tLLCP_DLCB *p_dlcb)
{
    /* if second continue comes */
    LLCP_TRACE_ERROR3("llcp_dlc_proc_i_pdu (): %d %d %d ", p_i_payload[0], p_i_payload[1], p_i_payload[2]);
    if ((p_i_payload[1] == 0x80 )||(p_i_payload[1] == 0x00 )) {
        if (!p_dlcb->i_error_flag) {
            p_dlcb->i_error_flag = TRUE;
            LLCP_TRACE_DEBUG0("llcp_dlc_proc_i_param i_error_flag = TRUE");
            return FALSE;
        } else {
            return TRUE;
        }
    } else if (p_i_payload[1] == 0x81) {
        p_dlcb->i_error_flag = FALSE;
        return FALSE;
    }
    return FALSE;
