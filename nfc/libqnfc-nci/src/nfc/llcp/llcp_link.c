...
static void llcp_link_deactivated_rsp (void);
...
tLLCP_STATUS llcp_link_activate (tLLCP_ACTIVATE_CONFIG *p_config)
{
                                   ...
                                   p_config->p_gen_bytes ) == FALSE)
    {
        ...
        if (in_llcp_or_snep_dta_mode()) {
            /* In DTA mode we first call llcp_link_send_invalid_pdu() and then
             * we call the callback which sends nci_deactivate */
...
#ifdef DTA // <DTA>
    /* internal delays are too big for NFC Forum LLCP testing,
     * we shouldn't add delay */
    if (!in_llcp_or_snep_dta_mode())
    {
#endif
...
void llcp_link_deactivate (UINT8 reason)
{
    ...
    {
        /* if received DISC to deactivate LLCP link as target role, send NFC DEP Information PDU */
        llcp_link_deactivated_rsp();
    }
    else if (in_llcp_or_snep_dta_mode() && reason == LLCP_LINK_RF_TIMEOUT) {
        /* the endpoind does not respond anymore, deactivate rf */
        (*llcp_cb.lcb.p_link_cback) (LLCP_LINK_DEACTIVATED_EVT, LLCP_LINK_RF_TIMEOUT);
    }
                                   ...
                                   BT_HDR *p_msg)
{
        ...
        &&(p_app_cb->link_type & LLCP_LINK_TYPE_LOGICAL_DATA_LINK)  )
    {
        ...
        if (!p_ui_pdu)
        {
            LLCP_TRACE_ERROR0 ("llcp_link_proc_ui_pdu (): error accessing p_ui_pdu");
            return;
        }
...
static void llcp_link_proc_rx_pdu (UINT8 dsap, UINT8 ptype, UINT8 ssap, BT_HDR *p_msg)
{
    ...
    case LLCP_PDU_DISC_TYPE:
        ...
        {
            LLCP_TRACE_DEBUG0 ("llcp_link_proc_rx_pdu (); link deactivation");
            llcp_link_deactivate (LLCP_LINK_REMOTE_INITIATED);
        }
        else
        {
...
static BT_HDR *llcp_link_build_next_pdu (BT_HDR *p_pdu)
{
    ...
    while (next_pdu_length > 0)
    {
        ...
        if (p_agf->len - LLCP_PDU_HEADER_SIZE + 2 + next_pdu_length <= llcp_cb.lcb.effective_miu)
        {
            ...
            if (p_next_pdu == NULL)
            {
                LLCP_TRACE_ERROR0 ("llcp_link_build_next_pdu (): Cannot get next pdu");
                break;
            }
...
void llcp_link_connection_cback (UINT8 conn_id, tNFC_CONN_EVT event, tNFC_CONN *p_data)
{
    if (event == NFC_DATA_CEVT)
    {
        ...
        {
            /* respoding with a NFC DEP Information PDU if RF link is not deactivated yet*/
            /* FIXME: it should be needed only when we are target */
            llcp_link_deactivated_rsp();
            GKI_freebuf ((BT_HDR *) p_data->data.p_data);
        }
        else if (llcp_cb.lcb.link_state == LLCP_LINK_STATE_ACTIVATION_FAILED)
        {
            /* responding with invalid LLC PDU until initiator deactivates RF link after LLCP activation was failed,
...
/*******************************************************************************
**
** Function         llcp_link_deactivated_rsp
**
** Description      Respond to outstanding and subsequent DEP-REQ I-PDUs
**                  while performing link deactivation
**
**
** Returns          None
**
...
static void llcp_link_deactivated_rsp (void)
{
    ...
    if (p_msg)
    {
        ...
        if (p)
        {
            /* FIXME: check for agreed version */
            if (LLCP_VERSION_MINOR == 0x01)
            {
                /* I-block containing the value 0000h */
                p_msg->len = 2;
                UINT16_TO_BE_STREAM(p, LLC_DEACTIVATED_PAYLOAD_V1_1);
            }
            else
            {
                /* I-block containing the value 0140h */
                p_msg->len = 2;
                UINT16_TO_BE_STREAM(p, LLC_DEACTIVATED_PAYLOAD_V1_2);
            }
            NFC_SendData (NFC_RF_CONN_ID, p_msg);
        }
        else
        {
            LLCP_TRACE_ERROR0 ("llcp_link_deactivated_rsp(): p == null");
        }
    }
    else
    {
        LLCP_TRACE_ERROR0 ("llcp_link_deactivated_rsp(): p_msg == null");
    }
