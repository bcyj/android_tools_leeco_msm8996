...
/*******************************************************************************
**
** Function         nfc_main_notify_poll_active_error
**
** Description      Notify error event in poll active mode
**
*******************************************************************************/
void nfc_main_notify_poll_active_error (tNFC_RESPONSE* nfc_rsp)
{
    ...
    if (nfc_rsp != NULL)
    {
        memcpy(&evt_data, nfc_rsp, sizeof(tNFC_RESPONSE));
        if (nfc_cb.p_resp_cback)
        {
            (*nfc_cb.p_resp_cback) (POLL_ACTIVE_ERR_REVT, &evt_data);
        }
    }
}
/*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
...
void nfc_main_handle_hal_evt (tNFC_HAL_EVT_MSG *p_msg)
{
    ...
    default:
        NFC_TRACE_ERROR1 ("nfc_main_handle_hal_evt (): ERROR: UNHANDLED EVENT (0x%x).", p_msg->hal_evt);
        break;
    }
}
                                        ...
                                        tNFC_DISCOVER_CBACK *p_cback)
{
    ...
    if(num_disc_maps > NFC_NFCC_MAX_NUM_VS_INTERFACE + NCI_INTERFACE_MAX) {
        NFC_TRACE_ERROR1 ("NFC_DiscoveryMap: Number of items %d in p_params invalid", num_disc_maps);
        num_disc_maps = NFC_NFCC_MAX_NUM_VS_INTERFACE + NCI_INTERFACE_MAX; // graceful degradation
    }
...
/*******************************************************************************
**
** Function         NFC_SendPresenceCheckCmd
**
** Description      This function sends presence check cmd to trigger presence check
**                  defined by ISO-DEP Method2
**
** Returns          tNFC_STATUS
**
*******************************************************************************/
tNFC_STATUS NFC_SendPresenceCheckCmd (void)
{
    NFC_TRACE_API0 ("NFC_SendPresenceCheckCmd ()");

    if (nfc_cb.nfc_state == NFC_STATE_OPEN)
    {
        return nci_snd_presence_check_cmd ();
    }

    NFC_TRACE_ERROR1 ("NFC_SendPresenceCheckCmd () invalid state = %d", nfc_cb.nfc_state);
    return NFC_STATUS_FAILED;
}
/* QNCI_FEATURE_ISODEP_PRESENCE */
