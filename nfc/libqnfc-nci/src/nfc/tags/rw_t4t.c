...
/*******************************************************************************
**
** Function         rw_t4t_send_presence_check_cmd
**
** Description      Send presence check cmd to NFCC
**
** Returns          TRUE if success
**
*******************************************************************************/
static BOOLEAN rw_t4t_send_presence_check_cmd (void)
{
    tNFC_STATUS status;

    RW_TRACE_DEBUG0 ("rw_t4t_send_presence_check_cmd ()");

    status = NFC_SendPresenceCheckCmd ();
...
/* QNCI_FEATURE_ISODEP_PRESENCE */
    case NFC_PRESENCE_CHECK_CEVT:
        if (p_t4t->state == RW_T4T_STATE_PRESENCE_CHECK)
        {
            if (p_data != NULL)
            {
                rw_data.status = *(tNFC_STATUS*)p_data;
            }
            else
            {
                rw_data.status = NFC_STATUS_FAILED;
                RW_TRACE_ERROR0 ("invalid param");
            }
        }
        break;
/* QNCI_FEATURE_ISODEP_PRESENCE */
    ...
    case RW_T4T_STATE_PRESENCE_CHECK:
        ...
        if (event != NFC_PRESENCE_CHECK_CEVT)
        {
            rw_data.status = NFC_STATUS_OK;
        }
        p_t4t->state = RW_T4T_STATE_IDLE;
...
tNFC_STATUS RW_T4tPresenceCheck (UINT8 option)
{
    ...
    else
    {
        ...
        /* QNCI_FEATURE_ISODEP_PRESENCE */
        else if (option == RW_T4T_CHK_NACK)
        {
            if (rw_t4t_send_presence_check_cmd())
            {
                status = TRUE;
            }
            else
        ...
        else/* QNCI_FEATURE_ISODEP_PRESENCE */
        {
