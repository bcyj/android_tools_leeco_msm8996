...
void rw_t3t_process_error (tNFC_STATUS status)
{
    ...
    /* Retransmit in case of timeout, we should check if it is needed for
       RF_TRANSMISSION_ERR of RF_PROTOCOL_ERR as well */
    if ((p_cb->rw_state == RW_T3T_STATE_COMMAND_PENDING) && ((status == NFC_STATUS_TIMEOUT) || (status == NFC_STATUS_MSG_CORRUPTED)))
    {
        ...
        {
            /* If doing presence check, use status=NFC_STATUS_FAILED, otherwise NFC_STATUS_TIMEOUT or NFC_STATUS_MSG_CORRUPTED */
...
void rw_t3t_handle_nci_poll_ntf (UINT8 nci_status, UINT8 num_responses, UINT8 sensf_res_buf_size, UINT8 *p_sensf_res_buf)
{
    ...
    {
        RW_TRACE_ERROR2 ("%s (%d)", __FUNCTION__, p_cb->rw_state);
    }
}
