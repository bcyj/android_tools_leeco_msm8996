...
enum
{
    ...
    NFC_NFCC_INFO_REVT                      /* NFCC information from ncihal     */
/*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
    ,POLL_ACTIVE_ERR_REVT                    /* error events in poll active mode */
};
...
enum
{
   ...
   ,NFC_PRESENCE_CHECK_CEVT                 /* 6  status of presence check      */
    /* QNCI_FEATURE_ISODEP_PRESENCE*/
};
...
{
    tNFC_STATUS             status;         /* The event status                 */
    UINT8                   numEntries;     /* number of LMRT Entries           */
    UINT8                   tlv_size;       /* the total len of all TLVs        */
    UINT8                   param_tlvs[255];/* the actual TLVs         */
...
/* error events in poll active mode data struct*/
typedef enum
{
    POLL_ACTIVE_ERR_REQ_DEACT,
    POLL_ACTIVE_ERR_RESEND_UICMD
}ePOLL_ACTIVE_ERR_STATE;

typedef struct
{
    ePOLL_ACTIVE_ERR_STATE state;
}tNFC_POLL_ACTIVE_ERR_REVT;
...
typedef union
{
    ...
    void                        *p_vs_evt_data;
    tNFC_NFCC_INFO              nfcc_info_data;      /*NFCC specific info*/
/*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
    tNFC_POLL_ACTIVE_ERR_REVT   error;    /* error events in poll active*/
