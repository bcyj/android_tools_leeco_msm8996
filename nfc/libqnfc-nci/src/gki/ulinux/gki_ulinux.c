...
#endif
            GKI_TRACE_2( "GKI_shutdown(): task %s dead,%d", gki_cb.com.OSTName[task_id-1],task_id-1 );
...
UINT16 GKI_wait (UINT16 flag, UINT32 timeout)
{
    ...
    GKI_TRACE_4("GKI_wait %d %x %d 0x%x", rtask, flag, timeout, rtask);
    ...
    pthread_mutex_unlock(&gki_cb.os.thread_evt_mutex[rtask]);
    GKI_TRACE_4("GKI_wait %d 0x%x %d %x resumed", rtask, flag, timeout, evt);
...
UINT8 GKI_get_taskid (void)
{
    ...
    for (i = 0; i < GKI_MAX_TASKS; i++) {
        if (gki_cb.os.thread_id[i] == thread_id) {
            GKI_TRACE_3("GKI_get_taskid %x %d %d done", thread_id, i, GKI_MAX_TASKS);
            return(i);
        }
    }

    GKI_TRACE_1("GKI_get_taskid: thread id = %x,cannot find", thread_id);
...
#endif
{
    ...
    GKI_TRACE_3("GKI_disable,%d,%s,%d", task_id, _function_, _line_);
    #endif
    ...
    /*  } */
    #if  GKI_BUFFER_DEBUG
    GKI_TRACE_1("Leaving GKI_disable,%d", task_id);
    #endif
    return;
}
