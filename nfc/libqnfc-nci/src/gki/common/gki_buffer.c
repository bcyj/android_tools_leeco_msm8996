...
static void gki_init_free_queue (UINT8 id, UINT16 size, UINT16 total, void *p_mem)
{
    ...
    /* if total=0 a null dereference may occur */
    if(total == 0) return;
...
#if GKI_BUFFER_DEBUG
    LOGD("GKI_getbuf() requesting %d func:%s(line=%d),%d", size, _function_, _line_, p_cb->curr_total_no_of_pools);
#endif
...
#endif
{
    ...
    if(Q->cur_cnt < Q->total)
    {
        ...
        if(Q->p_first == 0)
        {
            /* gki_alloc_free_queue() failed to alloc memory */
#if GKI_BUFFER_DEBUG
            LOGD("GKI_getpoolbuf() fail alloc free queue");
#endif
            GKI_enable();
            return NULL;
        }
...
/*******************************************************************************
**
** Function         GKI_getbufstatus
**
** Description      Called by an application to return status of buffer.
**
...
**
*******************************************************************************/
UINT8 GKI_getbufstatus (void *p_buf)
{
    BUFFER_HDR_T    *p_hdr;

    if (p_buf == NULL)
    {
        return BUF_STATUS_INVALID;
    }
    else
    {
...
*******************************************************************************/
#if GKI_BUFFER_DEBUG
void GKI_freebuf_debug (void *p_buf, const char * _in_function_, int _in_line_)
#else
...
#if GKI_BUFFER_DEBUG
    LOGD("GKI_freebuf() freeing, %x, %x, func:%s(line=%d) freed in func:%s(line=%d)", p_buf, p_hdr, p_hdr->_function, p_hdr->_line, _in_function_, _in_line_);
#endif
...
#if GKI_BUFFER_DEBUG
    LOGD("GKI_freebuf() free, %x %x(%d of %d used)", p_buf , p_hdr, Q->cur_cnt, Q->total);
#endif
    GKI_enable();
