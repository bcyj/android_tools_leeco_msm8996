...
void nfc_hal_hci_enable (void)
{
    ...
    {
        if ((p_hci_netwk_cmd = (UINT8 *)GKI_getpoolbuf (NFC_HAL_NCI_POOL_ID)) == NULL)
        {
...
void nfc_hal_hci_init_complete (tHAL_NFC_STATUS status)
{
    ...
    /* Posting this event will eventually end up signalling the waiting HAL_SYNC client. */
    HAL_TRACE_DEBUG0 ("nfc_hal_hci_init_complete(): HAL_SYNC: Posting HAL_NFC_POST_INIT_CPLT_EVT to configure dm.");
