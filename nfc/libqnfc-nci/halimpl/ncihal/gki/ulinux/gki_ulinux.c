...
void GKI_init(void)
{
    ...
    gki_buffer_init();
    gki_timers_init();
    gki_cb.com.OSTicks = (UINT32) times(&time_buff);
    ...
    pthread_mutex_init(&p_os->gki_timerupdate_mutex, NULL);
...
UINT8 GKI_create_task (TASKPTR task_entry, UINT8 task_id, INT8 *taskname, UINT16 *stack, UINT16 stacksize, void* pCondVar, void* pMutex)
{
    ...
    GKI_TRACE_6( "HAL Leaving GKI_create_task %x %d %x %s %x %d",
              task_entry,
              task_id,
...
#endif
            GKI_TRACE_2( "HAL,GKI_shutdown(): task %s dead, %d", gki_cb.com.OSTName[task_id-1], task_id-1);
...
UINT16 GKI_wait (UINT16 flag, UINT32 timeout)
{
    ...
    GKI_TRACE_4("HAL,GKI_wait %d %x %d 0x%x", rtask, flag, timeout, rtask);
    ...
    pthread_mutex_unlock(&gki_cb.os.thread_evt_mutex[rtask]);
    GKI_TRACE_4("HAL,GKI_wait %d 0x%x %d %x resumed", rtask, flag, timeout, evt);
...
{
    GKI_TRACE_2("HAL,GKI_send_event %d %x", task_id, event);
...
UINT8 GKI_get_taskid (void)
{
    ...
    for (i = 0; i < GKI_MAX_TASKS; i++) {
        if (gki_cb.os.thread_id[i] == thread_id) {
            GKI_TRACE_3("HAL,GKI_get_taskid %x %d %d done", thread_id, i, GKI_MAX_TASKS);
            return(i);
        }
    }

    GKI_TRACE_1("HAL,GKI_get_taskid: thread id = %x,cannot find", thread_id);
...
#if  GKI_BUFFER_DEBUG
void GKI_enable_debug ( const char * _function_, int _line_)
#else
...
#if GKI_BUFFER_DEBUG
void GKI_disable_debug ( const char * _function_, int _line_)
#else
...
#endif
{
    ...
    task_id = GKI_get_taskid();
    GKI_TRACE_3("HAL,GKI_disable,%d,%s,%d", task_id, _function_, _line_);
    #endif
    ...
    /*  } */
    #if  GKI_BUFFER_DEBUG
    GKI_TRACE_1("HAL,Leaving GKI_disable,%d", task_id);
    #endif
    return;
}
