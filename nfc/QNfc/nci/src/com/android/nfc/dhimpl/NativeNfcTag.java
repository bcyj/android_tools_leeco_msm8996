...
public class NativeNfcTag implements TagEndpoint {
    static final boolean DBG = Debug.NativeNfcTag;
    ...
    class PresenceCheckWatchdog extends Thread {
        ...
        public void run() {
            ...
            // avoid deadlock
            // if isStopped, just wait for thread to finish. mIsPresent is already false
            if (!isStopped) {
    ...
    public synchronized void setIsPresent(boolean isPresent) {
        Log.d(TAG,"[DTA] mIsPresent set to: " + isPresent);
    ...
    public boolean hasTech(int tech) {
            ...
            for (int i = 0; i < mTechList.length; i++) {
                ...
                switch (mTechList[i]) {
                    ...
                    case TagTechnology.MIFARE_ULTRALIGHT: {
                        // <DTA>
                        // <DTA> In DTA mode do not try to detect UltraLight
                        boolean isUlc = false;
                        if (!DtaHelper.isInDtaMode()) {
                            isUlc = isUltralightC();
                        }
                        // </!DTA> in dta mode isUlc remains false
    ...
    public NdefMessage findAndReadNdef() {
        ...
        for (int techIndex = 0; techIndex < technologies.length; techIndex++) {
            ...
            byte[] buff = null;
            // TODO: This was wrong in old code, keep note if still working
            // <DTA>
