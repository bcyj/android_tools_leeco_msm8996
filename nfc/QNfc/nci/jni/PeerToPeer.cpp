...
bool PeerToPeer::createClient (tJNI_HANDLE jniHandle, UINT16 miu, UINT8 rw)
{
    ...
    //if i = sMax, client would remain null (first contition). Only to avoid seg faults
...
#ifdef DTA // <DTA>
           if (in_llcp_snl()) {
               ALOGD ("%s: Attempt to connect with snl", fn);
               nfaStat = NFA_P2pGetRemoteSap (pClient->mNfaP2pClientHandle, "urn:nfc:sn:dta-co-echo-out");
           }
            else
#endif
...
void PeerToPeer::nfaClientCallback (tNFA_P2P_EVT p2pEvent, tNFA_P2P_EVT_DATA* eventData)
{
    ...
    case NFA_P2P_SDP_EVT:
        ...
        if (in_llcp_or_snep_dta_mode()) {
            if ((pClient = sP2p.findClient (eventData->sdp.handle)) == NULL)
            {
                ALOGE ("%s: NFA_P2P_CONNECTED_EVT: can't find client: 0x%04x", fn, eventData->sdp.handle);
            }
            else
            {
                NFA_P2pConnectBySap (pClient->mNfaP2pClientHandle, eventData->sdp.remote_sap,
                           pClient->mClientConn->mMaxInfoUnit, pClient->mClientConn->mRecvWindow);
            }
            break;
        }
...
#ifdef DTA // <DTA>
   // llcp echo server should have SAP 12
   if (mServiceName.compare("urn:nfc:sn:dta-co-echo-in") == 0)
       serverSap = 0x12;
#endif// <DTA>
   {
...
#ifdef DTA // <DTA>
        if (in_llcp_or_snep_dta_mode() && (connection->mNfaConnHandle == NFA_HANDLE_INVALID)) {
            connection = allocateConnection(connJniHandle);
        }
#endif // <DTA>
