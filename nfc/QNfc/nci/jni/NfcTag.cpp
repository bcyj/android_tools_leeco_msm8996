...
/*******************************************************************************
**
** Function:        getRfInterface
**
** Description:     Get the RF interface of the current tag.
**
** Returns:         RF Interface
**
*******************************************************************************/
tNFA_INTF_TYPE NfcTag::getRfInterface()
{
    return mRfInterface;
}
...
/*******************************************************************************
**
** Function:        setRfInterface
**
** Description:     Set the RF interface of the current tag.
**
** Returns:         void
**
*******************************************************************************/
void NfcTag::setRfInterface(tNFA_INTF_TYPE type)
{
    mRfInterface = type;
}
    ...
    case NFC_PROTOCOL_T2T:
        ...
        // could be MifFare UL or Classic or Kovio
        {
            ...
            if ((tech_params.param.pa.nfcid1[0] == 0x04 &&
                 rfDetail.rf_tech_param.param.pa.sel_rsp == MIFARE_ULTRALIGHT_SAK) ||
                 rfDetail.rf_tech_param.param.pa.sel_rsp == MIFARE_CLASSIC_4K_SAK ||
                 rfDetail.rf_tech_param.param.pa.sel_rsp == MIFARE_CLASSIC_1K_SAK)
            {
                mNumTechList++;
                mTechHandles [mNumTechList] = rfDetail.rf_disc_id;
                mTechLibNfcTypes [mNumTechList] = rfDetail.protocol;
                ...
                memcpy (&(mTechParams[mNumTechList]), &(rfDetail.rf_tech_param), sizeof(rfDetail.rf_tech_param));
                // sak == 0X00 Mifare UL
                if (rfDetail.rf_tech_param.param.pa.sel_rsp == MIFARE_ULTRALIGHT_SAK)
                {
                    mTechList [mNumTechList] = TARGET_TYPE_MIFARE_UL; //is TagTechnology.MIFARE_ULTRALIGHT by Java API
                }
                //SAK == 0x08 (basically 4th bit ==1 5th and 6th will decide if its 1K or 4K)
                if((rfDetail.rf_tech_param.param.pa.sel_rsp == MIFARE_CLASSIC_1K_SAK) || ((rfDetail.rf_tech_param.param.pa.sel_rsp == MIFARE_CLASSIC_4K_SAK)))
                {
                    mTechList [mNumTechList] = TARGET_TYPE_MIFARE_CLASSIC; // tag tech is mifare classic . inform to nfcservice.
                    NfcTag::getInstance().mIsMifareClassicOp = true;
                    ALOGD ("%s:  NfcTag::getInstance().mIsMifareClassicOp=%X", fn,NfcTag::getInstance().mIsMifareClassicOp);
                }
            }
        }
        break;
...
void NfcTag::discoverTechnologies (tNFA_DISC_RESULT& discoveryData)
{
    ...
    {
        if (mNumTechList > MAX_NUM_TECHNOLOGY)
            mNumTechList = MAX_NUM_TECHNOLOGY;
        ...
        case NFC_DISCOVERY_TYPE_LISTEN_F_ACTIVE:
            {
                ...
                ***/
                ...
                #ifdef DTA // <DTA>
                if (in_dta_mode() && !in_llcp_or_snep_dta_mode() && isNfcDep) { // Avoid SEGFAULT in the code block below.
        ...
        case NFC_PROTOCOL_T2T:
            {
                if (mTechLibNfcTypes[i] == NFC_PROTOCOL_T1T)
                {
                    ...
                    memcpy(mTechParams [0].param.pa.nfcid1 , activationData.params.t1t.uid, NFA_T1T_CMD_UID_LEN);
                    mTechParams [0].param.pa.nfcid1_len = NFA_T1T_CMD_UID_LEN;
                }
...
/************************************************************************************************
**
** Function:     SendNfccInfo
**
** Description:  Forward nfcc information(recieved by ncihal) to the nfcservice.
**
** Returns:      None
**
************************************************************************************************/
void NfcTag::SendNfccInfo(struct nfc_jni_native_data *nat,UINT8* nfcc_info_data, UINT8 len)
{
    static const char fn [] = "NfcTag::SendNfccInfo()";
    JNIEnv* e = NULL;
    ...
    ScopedLocalRef<jobject> nfccinfoJavaArray(e, e->NewByteArray(len));
    if (nfccinfoJavaArray.get() == NULL)
    {
        ALOGE ("fail allocate array");
        return;
    }
    e->SetByteArrayRegion ((jbyteArray)nfccinfoJavaArray.get(), 0, len, (jbyte *)nfcc_info_data);
    if (e->ExceptionCheck())
    {
        e->ExceptionClear();
    ...
    // send NFCC info to nfcservice
    ALOGD ("%s: Send NFCC info to nfcservice", fn);
    e->CallVoidMethod(nat->manager, android::gCachedNfcManagerNotifyNfccInfo, nfccinfoJavaArray.get());
    if (e->ExceptionCheck()) {
        e->ExceptionClear();
        ALOGE ("%s: failed to send nfcc info to nfc service", fn);
    }
}
