...
import android.nfc.dta.tech.NfcDep; //DTA
 ...
 * <p>
 */
public final class TagDta implements Parcelable {
    final byte[] mId;
    final int[] mTechList;
    final String[] mTechStringList;
    final Bundle[] mTechExtras;
    final int mServiceHandle;  // for use by NFC service, 0 indicates a mock
    final INfcTagDta mTagService; // interface to NFC service, will be null if mock tag
     ...
     * @hide
     */
    public TagDta(byte[] id, int[] techList, Bundle[] techListExtras, int serviceHandle,
            INfcTagDta tagService) {
        if (techList == null) {
     ...
     * @hide
     */
    public static TagDta createMockTag(byte[] id, int[] techList, Bundle[] techListExtras) {
        // set serviceHandle to 0 and tagService to null to indicate mock tag
        return new TagDta(id, techList, techListExtras, 0, null);
    }
    ...
    //      implementation.
    public TagDta rediscover() throws IOException {
        ...
        try {
            TagDta newTag = mTagService.rediscover(getServiceHandle());
            if (newTag != null) {
                return newTag;
            } else {
    ...
    /** @hide */
    public INfcTagDta getTagService() {
        return mTagService;
    }
    ...
    public static final Parcelable.Creator<TagDta> CREATOR =
            new Parcelable.Creator<TagDta>() {
        @Override
        public TagDta createFromParcel(Parcel in) {
            INfcTagDta tagService;

            // Tag fields
            byte[] id = TagDta.readBytesWithNull(in);
            ...
            if (isMock == 0) {
                tagService = INfcTagDta.Stub.asInterface(in.readStrongBinder());
            }
            else {
                tagService = null;
            }

            return new TagDta(id, techList, techExtras, serviceHandle, tagService);
        }

        @Override
        public TagDta[] newArray(int size) {
            return new TagDta[size];
        }
    };
