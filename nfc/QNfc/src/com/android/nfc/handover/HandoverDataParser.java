...
public class HandoverDataParser {
    ...
    private static final boolean DBG = Debug.HandoverManager;
    ...
    static final int MAX_BT_RETRIES = 3;
    static final int BT_INIT_DELAY = 1500;
    ...
    NdefRecord createBluetoothOobDataRecord() {
        ...
        synchronized (mLock) {
            ...
            if (mLocalBluetoothAddress == null) {
                if (DBG) Log.d(TAG, "Invalid Bluetooth Address, attempt recovery");
                attemptBTRecovery();
            }
    ...
    private void attemptBTRecovery () {
        int retry = 0;
        while (retry < MAX_BT_RETRIES) {
            try {
                Thread.sleep(BT_INIT_DELAY);
