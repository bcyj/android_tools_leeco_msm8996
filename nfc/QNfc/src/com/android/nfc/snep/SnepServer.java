...
public final class SnepServer {
    ...
    private static final boolean DBG = Debug.SnepServer;
    ...
    static boolean handleRequest(SnepMessenger messenger, Callback callback) throws IOException {
        ...
        if (((request.getVersion() & 0xF0) >> 4) != SnepMessage.VERSION_MAJOR) {
            ...
            // <DTA>
            // SNEP Test [TC_S_BIT_BV_02] expects connection to be closed after "Unsupported Major version."
            if (DtaHelper.isInLlcpOrSnepMode()) {
                return false;
            }
            // </DTA>
        ...
        /* in case of RFU message*/
        // <DTA>
        } else if (DtaHelper.isInLlcpOrSnepMode()) {
            if (request.getField() > SnepMessage.REQUEST_PUT && request.getField() < SnepMessage.REQUEST_REJECT) {
                messenger.sendMessage(SnepMessage.getMessage(
                    SnepMessage.RESPONSE_BAD_REQUEST));
                if (DBG) Log.d(TAG, "RFU request (" + request.getField() +")");
                return false;
            } else {
            // <DTA>
            // SNEP Test [TC_S_BIT_BI_02] expects connection to be closed after "Bad Request."
                return false;
            }
            // </DTA>
        // </DTA>
                    ...
                    while (threadRunning) {
                        ...
                        if (communicationSocket != null) {
                            ...
                            if (DtaHelper.isInLlcpOrSnepMode()) {
                                ...
                                fragmentLength = (mFragmentLength == -1) ?
