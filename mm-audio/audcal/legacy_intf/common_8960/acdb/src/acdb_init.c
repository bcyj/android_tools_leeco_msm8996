...
/*===========================================================================
    ...
    $Header: //source/qcom/qct/multimedia2/Audio/audcal2/common/8k_PL/acdb/rel/4.2/src/acdb_init.c#1 $

    when        who     what, where, why
    ----------  ---     -----------------------------------------------------
	2012-07-03  kpavan	AFEcmn table updation on EFS override
   ...
   void              *status_data)
{
   ...
   case ACDB_OEM_INFORMATION:
      {         
         ...
         cmdOEMI.nBufferPointer = (uint8_t*)pData->pOemInformation;
   ...
   case ACDB_DATE_MODIFIED:
      {
         ...
         cmdDateMod.nBufferPointer = (uint8_t*)pData->pDateString;
   ...
   case ACDB_GLBTBL_DATA:
      ...
      break;
   case ACDB_GLBDEFS_DATA:
      {
         AcdbParseGlbDefsTblDataType* pData = (AcdbParseGlbDefsTblDataType*) status_data;
         AcdbGblDefsCmdType cmdGBDefs;
         cmdGBDefs.nParamId = pData->nParamId;
         cmdGBDefs.nBufferPointer = pData->pBuffer;
         cmdGBDefs.nBufferLength = pData->nBufferSize;

         cmd_result = acdb_ioctl (ACDB_CMD_SET_GLBDEFS_DATA,
                                  (uint8_t*)&cmdGBDefs, sizeof(cmdGBDefs),
                                  NULL, 0);
         if (cmd_result != ACDB_SUCCESS)
         {
            ACDB_DEBUG_LOG ("[ACDB Init]->Set Global Defs Table Failed [%d]\n", cmd_result);
         }
      }
      break;
	  case ACDB_AFE_CMN_DATA:
      {
         ...
         AcdbAfeCmnDataCmdType cmdafeCMN;
		 cmdafeCMN.nParamId=pData->nParamId;		
         cmdafeCMN.nBufferLength = pData->nBufferSize;
         cmdafeCMN.nDeviceId = pData->nDeviceId;
         cmdafeCMN.nAfeSampleRateId = pData->nAfeSampleRateId;
		 cmdafeCMN.nBufferPointer= pData->pBuffer;
		 cmdafeCMN.nModuleId= pData->nModuleId;

         cmd_result = acdb_ioctl (ACDB_CMD_SET_AFE_COMMON_DATA,
                                  (uint8_t*)&cmdafeCMN, sizeof (AcdbAfeCmnDataCmdType),
                                  NULL, 0);
         if (cmd_result != ACDB_SUCCESS)
         {
            ACDB_DEBUG_LOG ("[ACDB Init]->Set AFE CMN Table data Failed [%d]\n", cmd_result);
         }
      }
      break;
   case ACDB_ADIE_SIDETONE_DATA:
       {
           AcdbParseAdieSidetoneDataType* pData = (AcdbParseAdieSidetoneDataType*)status_data;
           AcdbAdieSidetoneDataCmdType cmdAdieSidetone;

           cmdAdieSidetone.nTxDeviceId = pData->nTxDeviceId;
           cmdAdieSidetone.nRxDeviceId = pData->nRxDeviceId;
           cmdAdieSidetone.nModuleId = pData->nModuleId;
           cmdAdieSidetone.nParamId = pData->nParamId;
           cmdAdieSidetone.nBufferLength = pData->nBufferSize;
           cmdAdieSidetone.nBufferPointer = pData->pBuffer;

           cmd_result = acdb_ioctl (ACDB_CMD_SET_ADIE_SIDETONE_DATA,
               (uint8_t*)&cmdAdieSidetone, sizeof (cmdAdieSidetone),
               NULL, 0);
           if (cmd_result != ACDB_SUCCESS)
           {
               ACDB_DEBUG_LOG ("[ACDB Init]->Set AdieSidetone Failed [%08X]\n", cmd_result);
           }
       }
       break;
	  default:
	   result=ACDB_NOTSUPPORTED;
       ACDB_DEBUG_LOG ("[ACDB Init]->Command not supported [%d]\n", cmd);
      break;
...
static int32_t InitializeAcdbFromFileSystem ()
{
   ...
   if (result == ACDB_INIT_SUCCESS)
   {
	  result = AcdbParseTopologyIDSLists(&context,acdbParseCallback);
	  (void) AcdbInitFileClose (context.fc);
