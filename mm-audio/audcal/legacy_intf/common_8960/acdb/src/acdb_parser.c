...
/*===========================================================================
    ...
    $Header: //source/qcom/qct/multimedia2/Audio/audcal2/common/8k_PL/acdb/rel/4.2/src/acdb_parser.c#2 $
...
#define ACDB_FCC_ADST   0x54534441
#define ACDB_FCC_ADSI   0x49534441

#define ACDB_FCC_GBTB1   0x42544247
#define ACDB_FCC_GBIO1   0x54444247


#define ACDB_FIELD_SIZE          4

#define ACDB_MAX_BUFFER_SIZE 8192
...
typedef struct _AcdbParseContext
{
   ...
   uint8_t bParseForGlobalTableOnly;
} AcdbParseContext;
                              ...
                              uint32_t bytesToRead)
{
   ...
   if(bytesToRead>ACDB_MAX_BUFFER_SIZE)
	{
		if (pDst != NULL)
		{
			ACDB_MEM_FREE (pDst);
   ...
   else if (bytesLeft < bytesToRead)
   {
	  ...
	  leftBytes= bytesToRead - bytesLeft;
	 ...
	 while(leftBytes>0)
     {
	  ...
	  if(leftBytes<ACDB_READ_BUFFER_SIZE)
      {
	  	  count=leftBytes;
      }
      else
      {
		  count=ACDB_READ_BUFFER_SIZE;
	  }
     
         // copy the reminder of the requested buffer into the destination.
	  memcpy (pDst + (bytesToRead - leftBytes), pContext->pCache, count);

	  pContext->nCacheOffset = count;
         pContext->nCacheEnd = data.bytesRead;
         pContext->nFileOffset += data.bytesRead;
         result = ACDB_SUCCESS;
	  leftBytes= leftBytes-(count);
      }
...
static int32_t parseGlobalData (AcdbParseContext* pContext, AcdbChunkHeader* pHdr)
{
   int32_t result = ACDB_GENERAL_FAILURE;

   AcdbParseGlbDefsTblDataType data;
   memset (&data, 0, sizeof (data));
   if (pHdr->length < 1 * sizeof (uint32_t))
   {
      result = ACDB_GENERAL_FAILURE;
   }
   else
   {
      (void) getNextAcdbData (pContext, (uint8_t*) &data.nParamId, sizeof (uint32_t));
      memset (pContext->pBuffer, 0, ACDB_MAX_BUFFER_SIZE);
      data.nBufferSize = pHdr->length - sizeof (uint32_t);
      data.pBuffer = pContext->pBuffer;
      (void) getNextAcdbData (pContext, data.pBuffer, data.nBufferSize);

      (void) pContext->pfnCallback (ACDB_GLBDEFS_DATA, pContext->pHandle, &data);
   }
...
static int32_t parseGlobalTable (AcdbParseContext* pContext, AcdbChunkHeader* pHdr)
{
   int32_t result = ACDB_SUCCESS;
   uint32_t offset = ACDB_FIELD_SIZE;
   uint32_t end = pHdr->length;
   ...
   while (offset < end && result == ACDB_SUCCESS && pContext->bExitParser == FALSE)
   {
      ...
      {
	  case ACDB_FCC_GBIO1:
         (void) parseGlobalData (pContext, &chunk);
         break;
      default:
         (void) skipUnknownChunks (pContext, &chunk);
         break;
   ...
   if (pContext->bParseForGlobalTableOnly == TRUE)
   {
      pContext->bExitParser = TRUE;
   }
...
static int32_t parseAdieSidetoneData (AcdbParseContext* pContext, AcdbChunkHeader* pHdr)
{
   int32_t result = ACDB_GENERAL_FAILURE;

   AcdbParseAdieSidetoneDataType data;
   memset (&data, 0, sizeof (data));
   if (pHdr->length < 4 * sizeof (uint32_t))
   {
      result = ACDB_GENERAL_FAILURE;
   ...
   else
   {
      ...
      (void) pContext->pfnCallback (ACDB_ADIE_SIDETONE_DATA, pContext->pHandle, &data);
   }

   return result;
}

static int32_t parseAdieSidetoneTable (AcdbParseContext* pContext, AcdbChunkHeader* pHdr)
{
   int32_t result = ACDB_SUCCESS;
   uint32_t offset = ACDB_FIELD_SIZE;
   uint32_t end = pHdr->length;
   ...
   while (offset < end && result == ACDB_SUCCESS && pContext->bExitParser == FALSE)
   {
      ...
      {
      case ACDB_FCC_ADSI:
         (void) parseAdieSidetoneData (pContext, &chunk);
         break;
      default:
         (void) skipUnknownChunks (pContext, &chunk);
         break;
         ...
         while (offset < end && result == ACDB_SUCCESS && pContext->bExitParser == FALSE)
         {
            ...
            case ACDB_FCC_LIST:
			   ...
			   case ACDB_FCC_AFT2:
				  ...
				  break;
               case ACDB_FCC_ADST:
                   (void) parseAdieSidetoneTable (pContext, &chunk);
                   break;
			   case ACDB_FCC_GBTB1:
				  if(pContext->bParseForGlobalTableOnly == TRUE)
				  {
					 (void) parseGlobalTable (pContext, &chunk);
				  }
				  else
				  {
...
int32_t AcdbParse (void *pHandle, AcdbParseCBFuncPtrType pfnCallback)
{
   ...
   if (result == ACDB_SUCCESS)
   {
      ...
      context.bParseForGlobalTableOnly = FALSE;
...
int32_t AcdbParseTopologyIDSLists (void *pHandle, AcdbParseCBFuncPtrType pfnCallback)
{
   int32_t result = ACDB_SUCCESS;
   AcdbParseContext context;
   result = InitAcdbContext (&context);
   if (result == ACDB_SUCCESS)
   {
      ...
      context.pfnCallback = pfnCallback;
      context.bParseForTargetVersionOnly = FALSE;
	  context.bParseForGlobalTableOnly = TRUE;
      context.bExitParser = FALSE;
