...
/*
   ...
   */
#include "acdb.h"
#include "../../../common_8960/acdb/src/acdb_private.h"
#include "acph.h"
#include "rtc_q6_intf.h"

#define ACPH_PREBUFFER_LENGH 0x2500
uint32_t ACPH_BUFFER_LENGTH=ACPH_PREBUFFER_LENGH;
/*
   -------------------------------
   |Macros                       |
   -------------------------------
   */
uint32_t ACPH_HALF_BUF_LEN = ACPH_PREBUFFER_LENGH/2 - sizeof(uint32_t);
...
/**
 * FUNCTION : create_error_resp_ex
 *
 * DESCRIPTION : Create a response error buffer with the response buffer length, specified error code 
 * and request command id.
 *
 * DEPENDENCIES : None
 *
 ...
 * SIDE EFFECTS : None
 */
void create_error_resp_ex (uint32_t data_length,
        uint32_t error_code,
        char_t *req_buf_ptr,
        char_t **resp_buf_ptr,
        uint32_t *resp_buf_length
        )
{
    ...
    *resp_buf_length = ACPH_ERROR_FRAME_LENGTH+4;
    resp_data_length = ACPH_ERROR_FRAME_LENGTH+4 - ACPH_HEADER_LENGTH;
    /**copy command id */
    memcpy(*resp_buf_ptr, req_buf_ptr, ACPH_COMMAND_ID_LENGTH);
#ifdef LOGE
    LOGE("[ACPH ERROR]->Command ID [%08]\n",*((uint32_t*)req_buf_ptr));
    ...
    LOGE("[ACPH ERROR]->error code [%08X]\n",error_code);
	    ...
	    /**copy response buffer length this is useful in case of memory problems */
    memcpy(*resp_buf_ptr + ACPH_HEADER_LENGTH + ACPH_ERROR_FLAG_LENGTH+ACPH_ERROR_CODE_LENGTH, 
           &data_length, 
           ACPH_ERROR_CODE_LENGTH);
#ifdef LOGE
    LOGE("[ACPH ERROR]->data_length [%08X]\n",data_length);
#endif
}
        ...
        )
{
    ...
    {
        /** ACDB failed*/
        create_error_resp_ex(acdb_result.nBytesUsedInBuffer,result_flag, 
                req_buf_ptr, 
                resp_buf_ptr, 
                resp_buf_length);
        return;
        ...
        )
{
    ...
    {
        /**command parameter missing*/
        create_error_resp_ex(acdb_result.nBytesUsedInBuffer,ACPH_ERR_INVALID_COMMAND, req_buf_ptr, resp_buf_ptr, resp_buf_length);
        return;
    }
    if (NULL == acph_main_buffer)
    {
...
/**
 * FUNCTION : get_afe_common_data
 *
 * DESCRIPTION : get afe common data
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void get_afe_common_data (
        char_t *req_buf_ptr,
        char_t **resp_buf_ptr,
        uint32_t *resp_buf_length
        )
{
    uint32_t nPageSize = ACPH_BUFFER_LENGTH - ACPH_ACDB_BUFFER_POSITION;
    AcdbAfeCmnDataCmdType acdb_command;
    AcdbQueryResponseType acdb_result;
    int32_t result_flag;
    uint32_t ulData_Length;
    memcpy(&ulData_Length,req_buf_ptr + ACPH_DATA_LENGTH_POSITION,
    ...
    memcpy(&(acdb_command.nDeviceId), 
            req_buf_ptr + ACPH_HEADER_LENGTH, 
            ACPH_CAL_DATA_UNIT_LENGTH);
    memcpy(&(acdb_command.nAfeSampleRateId), 
            req_buf_ptr + ACPH_HEADER_LENGTH + ACPH_CAL_DATA_UNIT_LENGTH, 
            ACPH_CAL_DATA_UNIT_LENGTH);
    memcpy(&(acdb_command.nModuleId), 
            req_buf_ptr + ACPH_HEADER_LENGTH + 2 * ACPH_CAL_DATA_UNIT_LENGTH, 
    ...
    acdb_command.nBufferPointer = acph_main_buffer + ACPH_ACDB_BUFFER_POSITION;
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_AFE_COMMON_DATA,
            (const uint8_t *)&acdb_command,
            sizeof(AcdbAfeCmnDataCmdType),
            (uint8_t *)&acdb_result,
            sizeof(AcdbQueryResponseType));
    if (ACDB_SUCCESS != result_flag)
    {
...
/**
 * FUNCTION : set_afe_common_data
 *
 * DESCRIPTION : set afe common data
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void set_afe_common_data (
        char_t *req_buf_ptr,
        char_t **resp_buf_ptr,
        uint32_t *resp_buf_length
        )
{
    ...
    memcpy((void *) acdb_command.nBufferPointer, 
            ...
            acdb_command.nBufferLength);
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_SET_AFE_COMMON_DATA,
            (const uint8_t *)&acdb_command,
            sizeof(AcdbAfeCmnDataCmdType),
            (uint8_t *)NULL, 0);
    if (ACDB_SUCCESS != result_flag)
...
/**
 * FUNCTION : get_afe_common_table
 *
 * DESCRIPTION : get afe common  table
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void get_afe_common_table (
        char_t *req_buf_ptr,
        char_t **resp_buf_ptr,
        uint32_t *resp_buf_length
        )
{
    uint32_t nPageSize = ACPH_BUFFER_LENGTH - ACPH_ACDB_BUFFER_POSITION;
    AcdbAfeCommonTableCmdType acdb_command;
    AcdbQueryResponseType acdb_result;
    int32_t result_flag;
    uint32_t ulData_Length;
    memcpy(&ulData_Length,req_buf_ptr + ACPH_DATA_LENGTH_POSITION,
    ...
    memcpy(&(acdb_command.nDeviceId), 
            req_buf_ptr + ACPH_HEADER_LENGTH, 
            ACPH_CAL_DATA_UNIT_LENGTH);
    memcpy(&(acdb_command.nSampleRateId), 
            req_buf_ptr + ACPH_HEADER_LENGTH + ACPH_CAL_DATA_UNIT_LENGTH, 
            ACPH_CAL_DATA_UNIT_LENGTH);
    acdb_command.nBufferLength = nPageSize;
    acdb_command.nBufferPointer = acph_main_buffer + ACPH_ACDB_BUFFER_POSITION;
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_AFE_COMMON_TABLE,
            (const uint8_t *)&acdb_command,
            sizeof(AcdbAfeCommonTableCmdType),
            (uint8_t *)&acdb_result,
            sizeof(AcdbQueryResponseType));
    if (ACDB_SUCCESS != result_flag)
    {
...
/**
 * FUNCTION : set_afe_common_table
 *
 * DESCRIPTION : set afe common  table
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void set_afe_common_table (
        char_t *req_buf_ptr,
        char_t **resp_buf_ptr,
        uint32_t *resp_buf_length
        )
{
    ...
    memcpy((void *) acdb_command.nBufferPointer, 
            ...
            acdb_command.nBufferLength);
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_SET_AFE_COMMON_TABLE,
            (const uint8_t *)&acdb_command,
            sizeof(AcdbAfeCommonTableCmdType),
            (uint8_t *)NULL, 0);
    if (ACDB_SUCCESS != result_flag)
        ...
        )
{
    ...
    {
        /** ACDB failed*/
        create_error_resp_ex( sizeof(uint32_t) + sizeof(uint32_t)
                            + acdb_result.nBytesUsedCoppBuffer
                            + acdb_result.nBytesUsedPoppBuffer,result_flag,
                req_buf_ptr, resp_buf_ptr, resp_buf_length);
        return;
    }
    else
...
/**
 * FUNCTION : get_afe_topology_list
 *
 * DESCRIPTION : get afe topology list
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void get_afe_topology_list (
        char_t *req_buf_ptr,
        char_t **resp_buf_ptr,
        uint32_t *resp_buf_length
        )
{
    ...
    acdb_command.nBufferPointer = acph_main_buffer + ACPH_ACDB_BUFFER_POSITION;
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_AFE_TOPOLOGY_LIST,
            (const uint8_t *)&acdb_command,
            sizeof(AcdbGeneralInfoCmdType),
            (uint8_t *)&acdb_result,
            sizeof(AcdbQueryResponseType));
...
/**
 * FUNCTION : get_device_channel_type_list
 *
 * DESCRIPTION : get device channel mapping info: L, R, RB, 
 * RL..
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 ...
 * SIDE EFFECTS : None
 */
static void get_device_channel_type_list (
        char_t *req_buf_ptr,
        char_t **resp_buf_ptr,
        uint32_t *resp_buf_length
        )
{
    ...
    acdb_command.nBufferPointer = acph_main_buffer + ACPH_ACDB_BUFFER_POSITION;
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_DEVICE_CHANNEL_TYPE_LIST,
            (const uint8_t *)&acdb_command,
            sizeof(AcdbGeneralInfoCmdType),
            (uint8_t *)&acdb_result,
            sizeof(AcdbQueryResponseType));
...
/**
 * FUNCTION : get_glbtbl_data
 *
 * DESCRIPTION : get global table data
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void get_glbtbl_data (char_t *req_buf_ptr,
                             char_t **resp_buf_ptr,
                             uint32_t *resp_buf_length
                             )
{
    uint32_t nPageSize = ACPH_BUFFER_LENGTH - ACPH_ACDB_BUFFER_POSITION;
    AcdbGblTblCmdType acdb_command;
    AcdbQueryResponseType acdb_result;
    int32_t result_flag;
    uint32_t ulData_Length;
    memcpy(&ulData_Length,req_buf_ptr + ACPH_DATA_LENGTH_POSITION,
    ...
    {
        /**command parameter missing*/
        create_suc_resp(ACPH_ERR_INVALID_COMMAND, req_buf_ptr, resp_buf_ptr, resp_buf_length);
        return;
    }
    if (NULL == acph_main_buffer)
    {
        /**not initilized*/
        create_suc_resp(ACPH_ERR_UNKNOWN_REASON, req_buf_ptr, resp_buf_ptr, resp_buf_length);
        return;
    }
    /**copy calibration ids to acdb command*/
    memcpy(&(acdb_command.nModuleId), 
    ...
    acdb_command.nBufferPointer = (uint8_t *)(acph_main_buffer + ACPH_ACDB_BUFFER_POSITION);
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_GLBTBL_DATA,
            (const uint8_t *)&acdb_command,
            sizeof(AcdbGblTblCmdType),
            (uint8_t *)&acdb_result,
            sizeof(AcdbQueryResponseType));
    if (ACDB_SUCCESS != result_flag)
    {
...
/**
 * FUNCTION : set_glbtbl_data
 *
 * DESCRIPTION : set global table data
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void set_glbtbl_data (char_t *req_buf_ptr,
                             char_t **resp_buf_ptr,
                             uint32_t *resp_buf_length
                             )
{
    ...
    memcpy((void *) acdb_command.nBufferPointer, 
            ...
            acdb_command.nBufferLength);
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_SET_GLBTBL_DATA,
            (const uint8_t *)&acdb_command,
            sizeof(AcdbGblTblCmdType),
            (uint8_t *)NULL, 0);
    if (ACDB_SUCCESS != result_flag)
        ...
        )
{
    ...
    {
        /** ACDB failed*/
        create_error_resp_ex(acdb_result.nBytesUsedInBuffer,result_flag, req_buf_ptr, resp_buf_ptr, resp_buf_length);
        return;
    }
    else
    {
...
/**
 * FUNCTION : get_common_device_info
 *
 * DESCRIPTION : Request device information from ACDB
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 ...
 * SIDE EFFECTS : None
 */
static void get_common_device_info (char_t *req_buf_ptr,
                                    char_t **resp_buf_ptr,
                                    uint32_t *resp_buf_length
                                    )
{
    uint32_t nPageSize = ACPH_BUFFER_LENGTH - ACPH_ACDB_BUFFER_POSITION;
    AcdbDeviceInfoCmnCmdType acdb_command;
    AcdbQueryResponseType acdb_result;
    int32_t result_flag;
    uint32_t ulData_Length;
    memcpy(&ulData_Length,req_buf_ptr + ACPH_DATA_LENGTH_POSITION,
    ...
    acdb_command.nBufferPointer = (uint8_t *)(acph_main_buffer + ACPH_ACDB_BUFFER_POSITION);
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_COMMON_DEVICE_INFO,
                             (const uint8_t *)&acdb_command,
                             sizeof(AcdbDeviceInfoCmnCmdType),
                             (uint8_t *)&acdb_result, 
                             sizeof(AcdbQueryResponseType));
    if (ACDB_SUCCESS != result_flag)
    {
...
/**
 * FUNCTION : get_target_specific_device_info
 *
 * DESCRIPTION : Request device information from ACDB
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 ...
 * SIDE EFFECTS : None
 */
static void get_target_specific_device_info (char_t *req_buf_ptr,
                                             char_t **resp_buf_ptr,
                                             uint32_t *resp_buf_length
                                             )
{
    uint32_t nPageSize = ACPH_BUFFER_LENGTH - ACPH_ACDB_BUFFER_POSITION;
    AcdbDeviceInfoTargetSpecificCmdType acdb_command;
    AcdbQueryResponseType acdb_result;
    int32_t result_flag;
    uint32_t ulData_Length;
    memcpy(&ulData_Length,req_buf_ptr + ACPH_DATA_LENGTH_POSITION,
    ...
    memcpy(&(acdb_command.nDeviceId), 
           req_buf_ptr + ACPH_HEADER_LENGTH, 
           ACPH_CAL_DATA_UNIT_LENGTH);
    /**copy device type to acdb command*/
    memcpy(&(acdb_command.nDeviceType), 
           req_buf_ptr + ACPH_HEADER_LENGTH + ACPH_CAL_DATA_UNIT_LENGTH, 
           ACPH_CAL_DATA_UNIT_LENGTH);
    acdb_command.nBufferLength = nPageSize;
    acdb_command.pBufferPointer = (uint8_t *)(acph_main_buffer + ACPH_ACDB_BUFFER_POSITION);
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_TARGET_SPECIFIC_DEVICE_INFO,
                             (const uint8_t *)&acdb_command,
                             sizeof(AcdbDeviceInfoTargetSpecificCmdType),
                             (uint8_t *)&acdb_result, 
                             sizeof(AcdbQueryResponseType));
    if (ACDB_SUCCESS != result_flag)
    {
...
/**
 * FUNCTION : get_vocproc_common_topology_id
 *
 * DESCRIPTION : get the vocproc common topology id
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void get_vocproc_common_topology_id (char_t *req_buf_ptr,
                                            char_t **resp_buf_ptr,
                                            uint32_t *resp_buf_length
                                            )
{
    AcdbGetVocProcTopIdCmdType acdb_command;
    int32_t result_flag;
    uint32_t ulData_Length;
    memcpy(&ulData_Length,req_buf_ptr + ACPH_DATA_LENGTH_POSITION,
           ACPH_DATA_LENGTH_LENGTH);
    ...
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_VOCPROC_COMMON_TOPOLOGY_ID,
                             (const uint8_t *)&acdb_command,
                             sizeof(AcdbGetVocProcTopIdCmdType),
                             (uint8_t *)(acph_main_buffer + ACPH_ACDB_BUFFER_POSITION),
                             sizeof(AcdbGetTopologyIdRspType));
    if (ACDB_SUCCESS != result_flag)
    {
...
/**
 * FUNCTION : get_vocproc_common_topology_id_list
 *
 * DESCRIPTION : get the vocproc common topology id list
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void get_vocproc_common_topology_id_list (char_t *req_buf_ptr,
                                                 char_t **resp_buf_ptr,
                                                 uint32_t *resp_buf_length
                                                 )
{
    ...
    acdb_command.nBufferPointer = (uint8_t *)(acph_main_buffer + ACPH_ACDB_BUFFER_POSITION);
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_VOCPROC_COMMON_TOPOLOGY_ID_LIST,
                             (const uint8_t *)&acdb_command,
                             sizeof(AcdbGeneralInfoCmdType),
                             (uint8_t *)&acdb_result,
                             sizeof(AcdbQueryResponseType));
 ...
 * SIDE EFFECTS : None
 */
static void get_qact_data (char_t *req_buf_ptr,
                           char_t **resp_buf_ptr,
                           uint32_t *resp_buf_length
                           )
{
    ...
    acdb_command.nBufferPointer = (uint8_t *)(acph_main_buffer + ACPH_ACDB_BUFFER_POSITION);
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_QACT_INFO,
                             (const uint8_t *)&acdb_command,
                             sizeof(AcdbGeneralInfoCmdType),
                             (uint8_t *)&acdb_result,
                             sizeof(AcdbQueryResponseType));
...
/**
 * FUNCTION : get_compatible_remote_deviceid
 *
 * DESCRIPTION : get the remote device id
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void get_compatible_remote_deviceid (char_t *req_buf_ptr,
                                            char_t **resp_buf_ptr,
                                            uint32_t *resp_buf_length
                                            )
{
    AcdbGetRmtCompDevIdCmdType acdb_command;
    int32_t result_flag;
    uint32_t ulData_Length;
    memcpy(&ulData_Length,req_buf_ptr + ACPH_DATA_LENGTH_POSITION,
           ACPH_DATA_LENGTH_LENGTH);
    ...
    /**copy calibration ids to acdb command*/
    memcpy(&(acdb_command.nNativeDeviceId), 
            req_buf_ptr + ACPH_HEADER_LENGTH, 
            ACPH_CAL_DATA_UNIT_LENGTH);

    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_COMPATIBLE_REMOTE_DEVICE_ID,
                             (const uint8_t *)&acdb_command,
                             sizeof(AcdbGetRmtCompDevIdCmdType),
                             (uint8_t *)(acph_main_buffer + ACPH_ACDB_BUFFER_POSITION),
                             sizeof(AcdbGetRmtCompDevIdRspType));
    if (ACDB_SUCCESS != result_flag)
    {
        /** ACDB failed*/
        create_error_resp(result_flag, req_buf_ptr, resp_buf_ptr, resp_buf_length);
 ...
 * FUNCTION : get_anc_device_pair_list
 *
 * DESCRIPTION : get Record device pair list
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void get_rec_device_pair_list (
        char_t *req_buf_ptr,
        char_t **resp_buf_ptr,
        uint32_t *resp_buf_length
        )
{
    ...
    acdb_command.nBufferPointer = acph_main_buffer + ACPH_ACDB_BUFFER_POSITION;
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_AUDIO_RECORD_PAIR_LIST,
            (const uint8_t *)&acdb_command,
            sizeof(AcdbGeneralInfoCmdType),
            (uint8_t *)&acdb_result,
            sizeof(AcdbQueryResponseType));
...
/**
 * FUNCTION : get_aud_vol_popp_module_list
 *
 * DESCRIPTION : get_aud_vol_popp_module_list
 *
 * DEPENDENCIES : ACDB needs to be available and initialized
 *
 * PARAMS:
 ...
 * SIDE EFFECTS : None
 */
static void get_aud_vol_popp_module_list (
        char_t *req_buf_ptr,
        char_t **resp_buf_ptr,
        uint32_t *resp_buf_length
        )
{
    ...
    acdb_command.nBufferPointer = acph_main_buffer + ACPH_ACDB_BUFFER_POSITION;
    /** call ACDB */
    result_flag = acdb_ioctl(ACDB_CMD_GET_AUD_VOL_POPP_MODULE_LIST,
            (const uint8_t *)&acdb_command,
            sizeof(AcdbGeneralInfoCmdType),
            (uint8_t *)&acdb_result,
            sizeof(AcdbQueryResponseType));
       ...
       //Online Calibration
        ...
        case ACPH_CMD_SET_AFE_DATA:
            *pfnRequested_Operation_Function = set_afe_data;
            break;
        case ACPH_CMD_GET_GLBTBL_DATA:
             *pfnRequested_Operation_Function = get_glbtbl_data;
             break;
        case ACPH_CMD_SET_GLBTBL_DATA:
             *pfnRequested_Operation_Function = set_glbtbl_data;
             break;
	case ACPH_CMD_GET_AFE_COMMON_DATA:
		*pfnRequested_Operation_Function = get_afe_common_data;
		break;
	case ACPH_CMD_SET_AFE_COMMON_DATA:
		*pfnRequested_Operation_Function = set_afe_common_data;
		break;
	case ACPH_CMD_GET_AFE_COMMON_TABLE:
		*pfnRequested_Operation_Function = get_afe_common_table;
		break;
	case ACPH_CMD_SET_AFE_COMMON_TABLE:
		*pfnRequested_Operation_Function = set_afe_common_table;
		break;
	case ACPH_CMD_GET_AFE_TOPOLOGY_LIST:
		*pfnRequested_Operation_Function = get_afe_topology_list;
		break;
	case ACPH_CMD_GET_DEVICE_CHANNEL_TYPE_LIST:
		*pfnRequested_Operation_Function = get_device_channel_type_list;
		break;
		case ACPH_CMD_GET_AUD_VOL_POPP_MODULE_LIST:
		*pfnRequested_Operation_Function = get_aud_vol_popp_module_list;
		break;
        case ACPH_CMD_QUERY_MAX_BUFFER_LENGTH:
            *pfnRequested_Operation_Function = query_max_buffer_length;
            break;
        case ACPH_CMD_GET_COMMON_DEVICE_INFO:
            *pfnRequested_Operation_Function = get_common_device_info;
            break;
        case ACPH_CMD_GET_TARGET_SPECIFIC_DEVICE_INFO:
            *pfnRequested_Operation_Function = get_target_specific_device_info;
            break;
        case ACPH_CMD_GET_VOCPROC_COMMON_TOPOLOGY_ID:
            *pfnRequested_Operation_Function = get_vocproc_common_topology_id;
            break;
        case ACPH_CMD_GET_VOCPROC_COMMON_TOPOLOGY_ID_LIST:
            *pfnRequested_Operation_Function = get_vocproc_common_topology_id_list;
            break;
        case ACPH_CMD_GET_QACT_INFO:
            *pfnRequested_Operation_Function = get_qact_data;
            break;
	case ACPH_CMD_GET_COMPATIBLE_REMOTE_DEVICE_ID:
		*pfnRequested_Operation_Function = get_compatible_remote_deviceid;
		break;
	 case ACPH_CMD_GET_REC_DEVICE_PAIR_LIST:
            *pfnRequested_Operation_Function = get_rec_device_pair_list;
		break;			
        //Adie RTC
        ...
        case ACPH_CMD_RTC_TRANS_APR_SET_PKT_SHMEM:
            *pfnRequested_Operation_Function = rtc_trans_apr_set_pkt_shmem;
            break;
        default:
            LOGE("[ACDB]->Command ID [%08x], unknown command\n",*command_id);
            bResult = FALSE;
            break;
    }
        ...
        )
{
	...
	if(ACPH_BUFFER_LENGTH>ACPH_PREBUFFER_LENGH)
	{
		if(acph_main_buffer!=NULL)
		{
			free(acph_main_buffer);
			acph_main_buffer=NULL;
		}
		acph_main_buffer = (char_t *) malloc(ACPH_ACDB_BUFFER_POSITION + ACPH_BUFFER_LENGTH);
		ACPH_BUFFER_LENGTH=ACPH_PREBUFFER_LENGH;
	}
	
    pfnRequested_Operation_Function(req_buf_ptr, resp_buf_ptr, resp_buf_length);
	memcpy(&error_code,*resp_buf_ptr + ACPH_HEADER_LENGTH + ACPH_ERROR_FLAG_LENGTH,ACPH_ERROR_CODE_LENGTH);
	if(error_code==ACDB_INSUFFICIENTMEMORY)
	{
		memcpy(&bufferSize,*resp_buf_ptr + ACPH_HEADER_LENGTH + ACPH_ERROR_FLAG_LENGTH+4,  ACPH_ERROR_CODE_LENGTH);
		bufferSize+=ACPH_ACDB_BUFFER_POSITION;
		if(acph_main_buffer!=NULL)
		{
			free(acph_main_buffer);
			acph_main_buffer=NULL;
		}
		acph_main_buffer = (char_t *) malloc(bufferSize);
		if(acph_main_buffer!=NULL)
		{
			ACPH_BUFFER_LENGTH=bufferSize;
    pfnRequested_Operation_Function(req_buf_ptr, resp_buf_ptr, resp_buf_length);
		}
	}	
